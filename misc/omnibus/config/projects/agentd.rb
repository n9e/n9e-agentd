require "./lib/ostools.rb"

name 'agentd'
package_name 'n9e-agentd'

homepage 'https://n9e.didiyun.com/'

if ohai['platform'] == "windows"
  # Note: this is the path used by Omnibus to build the agent, the final install
  # dir will be determined by the Windows installer. This path must not contain
  # spaces because Omnibus doesn't quote the Git commands it launches.
  install_dir "C:/opt/n9e-agentd/"
  #python_2_embedded "#{install_dir}/embedded2"
  python_3_embedded "#{install_dir}/embedded3"
  maintainer 'yubo' # Windows doesn't want our e-mail address :(
else
  if redhat? || suse?
    maintainer 'yubo <steveyubo@didichuxing.com>'
  else
    maintainer 'yubo <steveyubo@didichuxing.com>'
  end

  if osx?
    unless ENV['SKIP_SIGN_MAC'] == 'true'
    #code_signing_identity ''
    end
    if ENV['HARDENED_RUNTIME_MAC'] == 'true'
      entitlements_file "#{files_path}/macos/Entitlements.plist"
    end
  end

  install_dir '/opt/n9e-agentd'
end

# build_version is computed by an invoke command/function.
# We can't call it directly from there, we pass it through the environment instead.
build_version "#{ENV['PACKAGE_VERSION']}"

build_iteration "#{ENV['PACKAGE_RELEASE']}"

description 'Nightingale Monitoring Agent
 The Nightingale Monitoring Agent is a lightweight process that monitors system
 processes and services, and sends information back to your Nightingale account.
 .
 This package installs and runs the advanced Agent daemon, which queues and
 forwards metrics from your applications as well as system services.
 .
 See https://n9e.didiyun.com/ for more information
'

# ------------------------------------
# Generic package information
# ------------------------------------

# .deb specific flags
package :deb do
  vendor 'yubo <steveyubo@didichuxing.com>'
  epoch 1
  license 'Apache License Version 2.0'
  section 'utils'
  priority 'extra'
  if ENV.has_key?('DEB_SIGNING_PASSPHRASE') and not ENV['DEB_SIGNING_PASSPHRASE'].empty?
    signing_passphrase "#{ENV['DEB_SIGNING_PASSPHRASE']}"
    if ENV.has_key?('DEB_GPG_KEY_NAME') and not ENV['DEB_GPG_KEY_NAME'].empty?
      gpg_key_name "#{ENV['DEB_GPG_KEY_NAME']}"
    end
  end
end

# .rpm specific flags
package :rpm do
  vendor 'yubo <steveyubo@didichuxing.com>'
  epoch 1
  dist_tag ''
  license 'Apache License Version 2.0'
  category 'System Environment/Daemons'
  priority 'extra'
  if ENV.has_key?('RPM_SIGNING_PASSPHRASE') and not ENV['RPM_SIGNING_PASSPHRASE'].empty?
    signing_passphrase "#{ENV['RPM_SIGNING_PASSPHRASE']}"
  end
end

# OSX .pkg specific flags
package :pkg do
  identifier 'com.didiyun.agent'
  unless ENV['SKIP_SIGN_MAC'] == 'true'
    signing_identity 'Developer ID Installer: DiDi Global Inc.'
  end
end
compress :dmg do
  window_bounds '200, 200, 584, 632'

  pkg_position '10, 10'
end

# Windows .msi specific flags
package :zip do
  if windows_arch_i386?
    skip_packager true
  else
    extra_package_dirs [
      "#{Omnibus::Config.source_dir()}\\etc\\n9e-agentd\\extra_package_files",
      "#{Omnibus::Config.source_dir()}\\cf-root",
    ]

    additional_sign_files [
        "#{Omnibus::Config.source_dir()}\\cf-root\\bin\\agentd.exe",
        "#{Omnibus::Config.source_dir()}\\cf-root\\bin\\lib-agent-three.dll"
      ]
    if ENV['SIGN_PFX']
      signing_identity_file "#{ENV['SIGN_PFX']}", password: "#{ENV['SIGN_PFX_PW']}", algorithm: "SHA256"
    end
  end
end

package :msi do

  # For a consistent package management, please NEVER change this code
  arch = "x64"
  if windows_arch_i386?
    upgrade_code '2b1c67e8-21a5-431d-86d3-806a2c2a5e70'
    arch = "x86"
  else
    upgrade_code 'e17ffac9-3c8b-4ff0-bd8d-637a4e8df905'
  end
  wix_candle_extension 'WixUtilExtension'
  wix_light_extension 'WixUtilExtension'
  extra_package_dir "#{Omnibus::Config.source_dir()}\\etc\\n9e-agentd\\extra_package_files"

  additional_sign_files_list = [
      "#{Omnibus::Config.source_dir()}\\n9e-agentd\\src\\github.com\\n9e\\n9e-agentd\\bin\\agentd\\agentd.exe"
    ]
  #if ENV['SIGN_WINDOWS']
  #  signing_identity "ECCDAE36FDCB654D2CBAB3E8975AA55469F96E4C", machine_store: true, algorithm: "SHA256"
  #end
  if ENV['SIGN_PFX']
    signing_identity_file "#{ENV['SIGN_PFX']}", password: "#{ENV['SIGN_PFX_PW']}", algorithm: "SHA256"
  end
  include_sysprobe = "false"
  additional_sign_files additional_sign_files_list
  parameters({
    'InstallDir' => install_dir,
    'InstallFiles' => "#{Omnibus::Config.source_dir()}/n9e-agentd/n9e-agentd/packaging/n9e-agentd/win32/install_files",
    'BinFiles' => "#{Omnibus::Config.source_dir()}/n9e-agentd/src/github.com/n9e/n9e-agentd/bin/agentd",
    'EtcFiles' => "#{Omnibus::Config.source_dir()}\\etc\\n9e-agentd",
    'IncludePython2' => "#{with_python_runtime? '2'}",
    'IncludePython3' => "#{with_python_runtime? '3'}",
    'Platform' => "#{arch}",
    'IncludeSysprobe' => "#{include_sysprobe}",
  })
end

# ------------------------------------
# Dependencies
# ------------------------------------

# Linux-specific dependencies
if linux?
  dependency 'procps-ng'
  dependency 'curl'
end

# creates required build directories
dependency 'n9e-agentd-prepare'

# n9e agentd
if ENV['MAKE_MOCK'] == 'true'
  dependency 'n9e-agentd-mock'
else
  dependency 'n9e-agentd'
end

# Additional software
if windows?
  if ENV['WINDOWS_DDNPM_DRIVER'] and not ENV['WINDOWS_DDNPM_DRIVER'].empty?
    dependency 'n9e-windows-filter-driver'
  end
end
# Bundled cacerts file (is this a good idea?)
dependency 'cacerts'

if osx?
  dependency 'n9e-agentd-mac-app'
end

if with_python_runtime? "3"
  dependency 'datadog-agent-integrations-py3'
end

#if linux?
#  dependency 'n9e-security-agent-policies'
#end

# External agents
dependency 'jmxfetch'

# version manifest file
dependency 'version-manifest'

# this dependency puts few files out of the omnibus install dir and move them
# in the final destination. This way such files will be listed in the packages
# manifest and owned by the package manager. This is the only point in the build
# process where we operate outside the omnibus install dir, thus the need of
# the `extra_package_file` directive.
# This must be the last dependency in the project.
dependency 'n9e-agentd-finalize'
dependency 'n9e-cf-finalize'

if linux?
  extra_package_file '/etc/init/n9e-agentd.conf'
  systemd_directory = "/usr/lib/systemd/system"
  if debian?
    systemd_directory = "/lib/systemd/system"

    extra_package_file "/etc/init.d/n9e-agentd"
  end
  if suse?
    extra_package_file "/etc/init.d/n9e-agentd"
  end
  extra_package_file "#{systemd_directory}/n9e-agentd.service"
  extra_package_file '/etc/n9e-agentd/'
  extra_package_file '/var/log/n9e/'
end

exclude '\.git*'
exclude 'bundler\/git'

if windows?
  #
  # For Windows build, files need to be stripped must be specified here.
  #
  windows_symbol_stripping_file "#{Omnibus::Config.source_dir()}\\cf-root\\bin\\agentd.exe"
  windows_symbol_stripping_file "#{Omnibus::Config.source_dir()}\\n9e-agentd\\src\\github.com\\n9e\\n9e-agentd\\bin\\agentd\\agentd.exe"
end

if linux? or windows?
  # the stripper will drop the symbols in a `.debug` folder in the installdir
  # we want to make sure that directory is not in the main build, while present
  # in the debug package.
  strip_build true
  debug_path ".debug"  # the strip symbols will be in here
end
