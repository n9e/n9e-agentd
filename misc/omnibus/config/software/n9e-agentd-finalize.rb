...
build do
    ...
    block do
        # Conf files
        if windows?
            conf_dir_root = "#{Omnibus::Config.source_dir()}/etc"
            conf_dir = "#{conf_dir_root}/extra_package_files/EXAMPLECONFSLOCATION"
            mkdir conf_dir
            move "#{install_dir}/etc/agentd.yaml.example", conf_dir_root, :force=>true
            #if ENV['WINDOWS_DDNPM_DRIVER'] and not ENV['WINDOWS_DDNPM_DRIVER'].empty? and not windows_arch_i386?
            #  move "#{install_dir}/etc/n9e-agentd/system-probe.yaml.example", conf_dir_root, :force=>true
            #end
            move "#{install_dir}/etc/conf.d/*", conf_dir, :force=>true
            #delete "#{install_dir}/bin/agentd.exe"
            # TODO why does this get generated at all
            delete "#{install_dir}/bin/agentd.exe~"

            #remove unneccessary copies caused by blanked copy of bin to #{install_dir} in n9e-agentd recipe
            delete "#{install_dir}/bin/lib-agent-three.dll"
            #delete "#{install_dir}/bin/agent/lib-agent-two.dll"
            delete "#{install_dir}/bin/customaction.dll"
        ...
        if linux? || osx?
            # Setup script aliases, e.g. `/opt/n9e-agent/embedded/bin/pip` will
        ...
        if linux?
            ...
            mkdir "/etc/init"
            move "#{install_dir}/scripts/n9e-agentd.conf", "/etc/init"
            ...
            if debian?
                ...
                mkdir "/etc/init.d"
                move "#{install_dir}/scripts/n9e-agentd", "/etc/init.d"
            end
            if suse?
                mkdir "/etc/init.d"
                move "#{install_dir}/scripts/n9e-agentd", "/etc/init.d"
            ...
            move "#{install_dir}/scripts/n9e-agentd.service", systemd_directory

            # Move configuration files
            mkdir "/etc/n9e-agentd"
            #move "#{install_dir}/bin/agentd", "/usr/bin/n9e-agentd"
            move "#{install_dir}/etc/agentd.yaml.example", "/etc/n9e-agentd"
            move "#{install_dir}/etc/conf.d", "/etc/n9e-agentd", :force=>true

            # Move SELinux policy
            if debian? || redhat?
              move "#{install_dir}/etc/selinux/*", "/etc/n9e-agentd/selinux"
            end

            # Create empty directories so that they're owned by the package
            # (also requires `extra_package_file` directive in project def)
            mkdir "/etc/n9e-agentd/checks.d"
            mkdir "/var/log/n9e"

            # remove unused configs
            delete "/etc/n9e-agentd/conf.d/apm.yaml.default"
            delete "/etc/n9e-agentd/conf.d/process_agent.yaml.default"

            # remove windows specific configs
            delete "/etc/n9e-agentd/conf.d/winproc.d"
            ...
            # The `aerospike` client is not installed: /opt/n9e-agent/embedded/lib/python2.7/site-packages/aerospike.so: ELF load command address/offset not properly aligned
        ...
        if osx?
            ...
            if code_signing_identity
                ...
                command "codesign #{hardened_runtime}--force --timestamp --deep -s '#{code_signing_identity}' '#{install_dir}/N9e Agent.app'"
            end
        end
    end
end
