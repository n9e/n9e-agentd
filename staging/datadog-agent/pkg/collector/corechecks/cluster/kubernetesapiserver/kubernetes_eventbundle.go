...
import (
	...
	as "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/kubernetes/apiserver"
	"k8s.io/klog/v2"
...
func getHostProviderID(nodename string) string {
	...
	if err != nil {
		klog.Warningf("Can't create client to query the API Server: %v", err)
		return ""
	}

	node, err := as.GetNode(cl, nodename)
	if err != nil {
		klog.Warningf("Can't get node from API Server: %v", err)
		return ""
	}

	providerID := node.Spec.ProviderID
	if providerID == "" {
		klog.Warningf("ProviderID not found")
		return ""
	}
...
func getDDAlertType(k8sType string) metrics.EventAlertType {
	...
	default:
		klog.V(5).Infof("Unknown event type '%s'", k8sType)
		return metrics.EventAlertTypeInfo
	}
}
