...
import (
	...
	ksmstore "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/kubestatemetrics/store"
...
func nodeConditionTransformer(s aggregator.Sender, name string, metric ksmstore.DDMetric, hostname string, tags []string) {
	...
	if !found {
		klog.V(5).Infof("Couldn't find 'condition' label, ignoring service check for metric '%s'", name)
		return
	}
	...
	if !found {
		klog.V(5).Infof("Couldn't find 'status' label, ignoring service check for metric '%s'", name)
		return
	}
	...
	case "MemoryPressure":
		...
		eventStatus = statusForCondition(statusLabel, false)
	default:
		klog.V(6).Infof("Invalid 'condition' label '%s', ignoring service check for metric '%s'", condition, name)
		return
	}

	node, found := metric.Labels["node"]
	if !found {
		klog.V(5).Infof("Couldn't find 'node' label, ignoring service check for metric '%s'", name)
		return
	}
...
func statusForCondition(status string, positiveEvent bool) metrics.ServiceCheckStatus {
	...
	default:
		klog.V(6).Infof("Unknown 'status' label: '%s'", status)
		return metrics.ServiceCheckUnknown
	}
}
...
func nodeUnschedulableTransformer(s aggregator.Sender, name string, metric ksmstore.DDMetric, hostname string, tags []string) {
	...
	default:
		klog.V(5).Infof("Invalid metric value '%v', ignoring metric '%s'", metric.Val, name)
		return
	}
...
func submitContainerResourceMetric(s aggregator.Sender, name string, metric ksmstore.DDMetric, hostname string, tags []string, metricSuffix string) {
	...
	if !found {
		klog.V(5).Infof("Couldn't find 'resource' label, ignoring resource metric '%s'", name)
		return
	}
	...
	case "memory":
		...
		return
	default:
		klog.V(6).Infof("Ignoring container resource metric '%s': resource '%s' is not supported", name, resource)
	}
}
...
func submitNodeResourceMetric(s aggregator.Sender, name string, metric ksmstore.DDMetric, hostname string, tags []string, metricSuffix string) {
	...
	case "pods":
		...
		return
	default:
		klog.V(6).Infof("Ignoring node resource metric '%s': resource '%s' is not supported", name, resource)
	}
}
...
func resourcequotaTransformer(s aggregator.Sender, name string, metric ksmstore.DDMetric, hostname string, tags []string) {
	resource, found := metric.Labels["resource"]
	if !found {
		klog.V(5).Infof("Couldn't find 'resource' label, ignoring metric '%s'", name)
		return
	}
	quotaType, found := metric.Labels["type"]
	if !found {
		klog.V(5).Infof("Couldn't find 'type' label, ignoring metric '%s'", name)
		return
	}
	if quotaType == "hard" {
		quotaType = "limit"
...
func limitrangeTransformer(s aggregator.Sender, name string, metric ksmstore.DDMetric, hostname string, tags []string) {
	...
	if !found {
		klog.V(5).Infof("Couldn't find 'constraint' label, ignoring metric '%s'", name)
		return
	}
	constraint, found := constraintsMapper[constraintLabel]
	if !found {
		klog.V(5).Infof("Constraint '%s' unsupported for metric '%s'", constraint, name)
		return
	}
	resource, found := metric.Labels["resource"]
	if !found {
