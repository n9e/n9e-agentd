...
func retryFailedScalarOids(session sessionAPI, results *gosnmp.SnmpPacket, valuesToUpdate scalarResultValuesType) {
	...
	if len(retryOids) > 0 {
		...
		if err != nil {
			klog.V(5).Infof("failed to oids `%v` on retry: %v", retryOids, err)
		} else {
...
func doDoFetchScalarOids(session sessionAPI, oids []string) (*gosnmp.SnmpPacket, error) {
	klog.V(5).Infof("fetch scalar: request oids: %v", oids)
	results, err := session.Get(oids)
	if err != nil {
		klog.V(5).Infof("fetch scalar: error getting oids `%v`: %v", oids, err)
		return nil, fmt.Errorf("fetch scalar: error getting oids `%v`: %v", oids, err)
	}
	klog.V(5).Infof("fetch scalar: results: Variables=%v, Error=%v, ErrorIndex=%v", results.Variables, results.Error, results.ErrorIndex)
	return results, nil
}
