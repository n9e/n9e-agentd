...
func (c *ntpConfig) parse(data []byte, initData []byte, getLocalServers func() ([]string, error)) error {
	...
	if defaultHosts == nil {
		klog.V(5).Info("No cloud provider detected, using default ntp pool.")
	...
	if c.instance.UseLocalDefinedServers {
		...
		klog.Infof("Use local defined servers: %v", localNtpServers)
	}
...
func (c *NTPCheck) Configure(data integration.Data, initConfig integration.Data, source string) error {
	...
	if err != nil {
		klog.Errorf("Error parsing configuration file: %s", err)
		return err
	}
...
func (c *NTPCheck) queryOffset() (float64, error) {
	...
	for _, host := range c.cfg.instance.Hosts {
		...
		if err != nil {
			if c.errCount >= 10 {
				c.errCount = 0
				klog.Warningf("Couldn't query the ntp host %s for 10 times in a row: %s", host, err)
			} else {
				c.errCount++
				klog.V(5).Infof("There was an error querying the ntp host %s: %s", host, err)
			}
			continue
		}
		c.errCount = 0
		...
		if err != nil {
			klog.Infof("The ntp response is not valid for host %s: %s", host, err)
			continue
		}
