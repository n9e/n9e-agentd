...
func (c *Check) Run() error {
	...
	if err != nil {
		klog.V(5).Infof("cpu.Check could not read context switches: %s", err.Error())
	...
	if err != nil {
		klog.Errorf("cpu.Check: could not retrieve cpu stats: %s", err)
		return err
	} else if len(cpuTimes) < 1 {
		errEmpty := fmt.Errorf("no cpu stats retrieve (empty results)")
		klog.Errorf("cpu.Check: %s", errEmpty)
		return errEmpty
	}
	t := cpuTimes[0]
