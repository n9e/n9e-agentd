...
func (c *IOCheck) nixIO() error {
	...
	if err != nil {
		klog.Errorf("system.IOCheck: could not retrieve io stats: %s", err)
		return err
	}
	...
	for device, ioStats := range iomap {
		...
		if !ok {
			klog.V(5).Info("New device stats (possible hotplug) - full stats unavailable this iteration.")
			continue
		}

		if delta == 0 {
			klog.V(5).Info("No delta to compute - skipping.")
			continue
		}
