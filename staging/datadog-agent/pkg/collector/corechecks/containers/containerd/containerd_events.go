...
import (
	...
	ctrUtil "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containerd"
	"k8s.io/klog/v2"
	"k8s.io/klog/v2"
)
...
func (s *subscriber) CheckEvents(ctrItf ctrUtil.ContainerdItf) {
	...
	ev := ctrItf.GetEvents()
	klog.Info("Starting routine to collect Containerd events ...")
	...
	for {
		...
		case message := <-stream:
			...
			case "/containers/create":
				...
				if err != nil {
					klog.Errorf("Could not process create event from Containerd: %v", err)
					continue
				}
			...
			case "/containers/delete":
				...
				if err != nil {
					klog.Errorf("Could not process delete event from Containerd: %v", err)
					continue
				}
			...
			case "/containers/update":
				...
				if err != nil {
					klog.Errorf("Could not process update event from Containerd: %v", err)
					continue
				}
			...
			case "/tasks/exit":
				...
				if err != nil {
					klog.Errorf("Could not process exit event from Containerd: %v", err)
					continue
				}
			...
			case "/tasks/resumed":
				...
				s.addEvents(event)
			default:
				klog.V(6).Infof("Unsupported event type from Containerd: %s ", message.Topic)
			}
		case e := <-errC:
			...
			if e == context.Canceled {
				klog.V(5).Infof("Context of the event listener routine was canceled")
				return nil
			}
			klog.Errorf("Error while streaming logs from containerd: %s", e.Error())
...
func (s *subscriber) Flush(timestamp int64) []containerdEvent {
	...
	if len(s.Events) == 0 {
		klog.V(6).Infof("No events collected in the last %d seconds", delta)
		return nil
	}
	s.CollectionTimestamp = timestamp
	ev := s.Events
	klog.V(5).Infof("Collecting %d events from Containerd", len(ev))
	s.Events = nil
	return ev
}
