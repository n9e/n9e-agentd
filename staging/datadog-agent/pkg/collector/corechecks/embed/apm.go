...
func (c *APMCheck) run() error {
	...
	case <-c.stop:
		klog.Info("Not starting APM check: stop requested")
		c.stopDone <- struct{}{}
		return nil
	default:
	}
	...
	case <-c.stop:
		...
		if err != nil {
			klog.Errorf("unable to stop APM check: %s", err)
		}
	}
...
func (c *APMCheck) Configure(data integration.Data, initConfig integration.Data, source string) error {
	...
	if checkConf.BinPath != "" {
		if _, err := os.Stat(checkConf.BinPath); err == nil {
			...
			klog.Warningf("Can't access apm binary at %s, falling back to default path at %s", checkConf.BinPath, defaultBinPath)
		}
	}
...
func (c *APMCheck) Stop() {
	if atomic.LoadUint32(&c.running) == 0 {
		klog.Info("APM Agent not running.")
		return
	}
