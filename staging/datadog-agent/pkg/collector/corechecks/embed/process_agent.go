...
func (c *ProcessAgentCheck) run() error {
	...
	case <-c.stop:
		klog.Info("Not starting Process Agent check: stop requested")
		c.stopDone <- struct{}{}
		return nil
	default:
	}
	...
	case <-c.stop:
		...
		if err != nil {
			klog.Errorf("unable to stop process-agent check: %s", err)
		}
	}
...
func (c *ProcessAgentCheck) Configure(data integration.Data, initConfig integration.Data, source string) error {
	...
	if enabled := config.Datadog.GetBool("process_config.enabled"); !enabled {
		klog.Info("live process monitoring is disabled through main configuration file")
	}
	...
	if checkConf.BinPath != "" {
		if _, err := os.Stat(checkConf.BinPath); err == nil {
			...
			klog.Warningf("Can't access process-agent binary at %s, falling back to default path at %s", checkConf.BinPath, defaultBinPath)
		}
	}
...
func (c *ProcessAgentCheck) Stop() {
	if atomic.LoadUint32(&c.running) == 0 {
		klog.Info("Process Agent not running.")
		return
	}
