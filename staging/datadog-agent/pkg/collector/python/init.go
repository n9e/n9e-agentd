...
func detectPythonLocation(pythonVersion string) {
	...
	if runtime.GOOS == "windows" {
		...
		if err != nil {
			klog.Warningf("Error getting executable folder: %v", err)
			klog.Warningf("Trying again allowing symlink resolution to fail")
			_here, err = executable.FolderAllowSymlinkFailure()
			if err != nil {
				klog.Warningf("Error getting executable folder w/o symlinks: %v", err)
			}
		}
		klog.V(5).Infof("Executable folder is %v", _here)
		...
		if _, err := os.Stat(agentpythonHome2); os.IsNotExist(err) {
			klog.Warningf("relative embedded directory not found for python2; using default %s", pythonHome2)
		} else {
			pythonHome2 = agentpythonHome2
		}
		if _, err := os.Stat(agentpythonHome3); os.IsNotExist(err) {
			klog.Warningf("relative embedded directory not found for python3; using default %s", pythonHome3)
		} else {
...
func Initialize(paths ...string) error {
	...
	if initializePlatform() != nil {
		klog.Warningf("unable to complete platform-specific initialization - should be non-fatal")
	}
	...
	if pythonVersion == "2" {
		...
		klog.Infof("Initializing rtloader with python2 %s", PythonHome)
	} else if pythonVersion == "3" {
		rtloader = C.make3(csPythonHome, &pyErr)
		klog.Infof("Initializing rtloader with python3 %s", PythonHome)
	} else {
	...
	if pyInfo != nil {
		...
		klog.Errorf("Could not query python information: %s", C.GoString(C.get_error(rtloader)))
	}
...
func Destroy() {
	...
	if rtloader == nil {
		klog.Warning("Python runtime already destroyed. Ignoring action.")
		return
	}

	// Clear the C-side and Go-side rtloader pointers
	klog.Info("Destroying Python runtime")
	C.destroy(rtloader)
	rtloader = nil

	klog.Info("Python runtime destroyed")
}
