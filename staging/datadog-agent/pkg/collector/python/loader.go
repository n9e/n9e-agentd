...
func (cl *PythonCheckLoader) Load(config integration.Config, instance integration.Data) (check.Check, error) {
	...
	if !agentConfig.Datadog.GetBool("win_skip_com_init") {
		klog.V(5).Infof("Performing platform loading prep")
		...
		klog.Infof("Skipping platform loading prep")
	}
	...
	for _, name = range modules {
		...
		if err = getRtLoaderError(); err != nil {
			klog.V(5).Infof("Unable to load python module - %s: %v", name, err)
		} else {
			klog.V(5).Infof("Unable to load python module - %s", name)
		}
	}

	// all failed, return error for last failure
	if checkModule == nil || checkClass == nil {
		klog.V(5).Infof("PyLoader returning %s for %s", err, moduleName)
		return nil, err
	}
	...
	if res := C.get_attr_string(rtloader, checkModule, versionAttr, &version); res != 0 {
		...
		klog.V(5).Infof("python check '%s' doesn't have a '__version__' attribute: %s", config.Name, getRtLoaderError())
	}
	...
	if !agentConfig.Datadog.GetBool("disable_py3_validation") && !loadedAsWheel {
		...
		if res := C.get_attr_string(rtloader, checkModule, fileAttr, &checkFilePath); res != 0 {
			...
			klog.V(5).Infof("Could not query the __file__ attribute for check %s: %s", name, getRtLoaderError())
		}
	...
	klog.V(5).Infof("python loader: done loading check %s (version %s)", moduleName, wheelVersion)
	return c, nil
}
...
func addExpvarConfigureError(check string, errMsg string) {
	klog.Errorf("py.loader: could not configure check '%s': %s", check, errMsg)
...
func reportPy3Warnings(checkName string, checkFilePath string) {
	...
	if checkFilePath != "" {
		...
		if strings.TrimSpace(config.Datadog.GetString("python_version")) == "3" {
			...
			if err != nil {
				status = a7TagUnknown
				klog.Warningf("Failed to validate Python 3 linting for check '%s': '%s'", checkName, err)
			} else if len(warnings) == 0 {
				status = a7TagReady
				metricValue = 1.0
			} else {
				status = a7TagNotReady
				klog.Warningf("The Python 3 linter returned warnings for check '%s'. For more details, check the output of the 'status' command or the status page of the Agent GUI).", checkName)
				statsLock.Lock()
				defer statsLock.Unlock()
				for _, warning := range warnings {
					klog.V(5).Info(warning)
