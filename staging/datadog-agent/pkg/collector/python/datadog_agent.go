...
func GetHostname(hostname **C.char) {
	...
	if err != nil {
		klog.Warningf("Error getting hostname: %s\n", err)
		goHostname = ""
	}
...
func Headers(yamlPayload **C.char) {
	...
	if err != nil {
		klog.Errorf("datadog_agent: could not Marshal headers: %s", err)
		*yamlPayload = nil
		return
	}
...
func GetConfig(key *C.char, yamlPayload **C.char) {
	...
	if err != nil {
		klog.Errorf("could not convert configuration value '%v' to YAML: %s", value, err)
		*yamlPayload = nil
		return
	}
...
func ReadPersistentCache(key *C.char) *C.char {
	...
	if err != nil {
		klog.Errorf("Failed to read cache %s: %s", keyName, err)
		return nil
	}
...
func lazyInitObfuscator() *obfuscate.Obfuscator {
	obfuscatorLoader.Do(func() {
		...
		if err := config.Datadog.UnmarshalKey("apm_config.obfuscation", &cfg); err != nil {
			klog.Errorf("Failed to unmarshal apm_config.obfuscation: %s", err.Error())
