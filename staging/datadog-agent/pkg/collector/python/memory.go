...
func MemoryTracker(ptr unsafe.Pointer, sz C.size_t, op C.rtloader_mem_ops_t) {
	...
	klog.V(6).Infof("Memory Tracker - ptr: %v, sz: %v, op: %v", ptr, sz, op)
	switch op {
	...
	case C.DATADOG_AGENT_RTLOADER_FREE:
		...
		if !ok {
			klog.V(5).Infof("untracked memory was attempted to be freed - set trace level for details")
			lvl, err := log.GetLogLevel()
			if err == nil && lvl == seelog.TraceLvl {
				stack := string(debug.Stack())
				klog.V(6).Infof("Memory Tracker - stacktrace: \n%s", stack)
			}
