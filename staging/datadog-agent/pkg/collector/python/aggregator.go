...
import (
	...
	chk "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/collector/check"
...
func SubmitMetric(checkID *C.char, metricType C.metric_type_t, metricName *C.char, value C.double, tags **C.char, hostname *C.char, flushFirstValue C.bool) {
	...
	if err != nil || sender == nil {
		klog.Errorf("Error submitting metric to the Sender: %v", err)
		return
	}
...
func eventParseString(value *C.char, fieldName string) string {
	if value == nil {
		klog.V(6).Infof("Can't parse value for key '%s' in event submitted from python check", fieldName)
		return ""
	}
...
func SubmitHistogramBucket(checkID *C.char, metricName *C.char, value C.longlong, lowerBound C.float, upperBound C.float, monotonic C.int, hostname *C.char, tags **C.char) {
	...
	sender, err := aggregator.GetSender(chk.ID(goCheckID))
	if err != nil || sender == nil {
		klog.Errorf("Error submitting histogram bucket to the Sender: %v", err)
		return
	}
