...
func (c *PythonCheck) runCheck(commitMetrics bool) error {
	...
	klog.V(5).Infof("Running python check %s %s", c.ModuleName, c.id)
...
func (c *PythonCheck) Cancel() {
	gstate, err := newStickyLock()
	if err != nil {
		klog.Warningf("failed to cancel check %s: %s", c.id, err)
		return
	}
	defer gstate.unlock()
...
func (c *PythonCheck) getPythonWarnings(gstate *stickyLock) []error {
	...
	**/
	...
	if pyWarnings == nil {
		if err := getRtLoaderError(); err != nil {
			klog.Errorf("error while collecting python check's warnings: %s", err)
		}
		return nil
	}
...
func (c *PythonCheck) Configure(data integration.Data, initConfig integration.Data, source string) error {
	...
	if err := yaml.Unmarshal(initConfig, &commonGlobalOptions); err != nil {
		klog.Errorf("invalid init_config section for check %s: %s", string(c.id), err)
		return err
	}
	...
	if len(commonGlobalOptions.Service) > 0 {
		s, err := aggregator.GetSender(c.id)
		if err != nil {
			klog.Errorf("failed to retrieve a sender for check %s: %s", string(c.id), err)
		} else {
	...
	if err := yaml.Unmarshal(data, &commonOptions); err != nil {
		klog.Errorf("invalid instance section for check %s: %s", string(c.id), err)
		return err
	}
	...
	if res == 0 {
		...
		klog.Warningf("could not get a '%s' check instance with the new api: %s", c.ModuleName, rtLoaderError)
		klog.Warning("trying to instantiate the check with the old api, passing agentConfig to the constructor")

		allSettings := config.Datadog.AllSettings()
		agentConfig, err := yaml.Marshal(allSettings)
		if err != nil {
			klog.Errorf("error serializing agent config: %s", err)
			return err
		}
		...
		klog.Warningf("passing `agentConfig` to the constructor is deprecated, please use the `get_config` function from the 'datadog_agent' package (%s).", c.ModuleName)
	}
	c.instance = check
	c.source = source
	...
	klog.V(5).Infof("python check configure done %s", c.ModuleName)
	return nil
}
...
func pythonCheckFinalizer(c *PythonCheck) {
	...
	go func(c *PythonCheck) {
		klog.V(5).Infof("Running finalizer for check %s", c.id)

		glock, err := newStickyLock() // acquire lock to call DecRef
		if err != nil {
			klog.Warningf("Could not finalize check %s: %s", c.id, err.Error())
			return
		}
		defer glock.unlock()
