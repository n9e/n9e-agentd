...
func (s *Scheduler) Enter(check check.Check) error {
	...
	klog.Infof("Scheduling check %v with an interval of %v", check, check.Interval())
...
func (s *Scheduler) Cancel(id check.ID) error {
	...
	klog.Infof("Unscheduling check %s", string(id))
...
func (s *Scheduler) Run() {
	...
	if atomic.LoadUint32(&s.running) != 0 {
		klog.V(5).Info("Scheduler is already running")
		return
	}

	go func() {
		klog.V(5).Info("Starting scheduler loop...")
		...
		klog.V(5).Info("Exited Scheduler loop, shutting down queues...")
		s.stopQueues()
...
func (s *Scheduler) Stop() error {
	...
	if atomic.LoadUint32(&s.running) == 0 {
		klog.V(5).Info("Scheduler is already stopped")
		return nil
	}
	...
	klog.V(5).Infof("Waiting for the scheduler to shutdown")
...
func (s *Scheduler) stopQueues() {
	...
	klog.V(5).Infof("Stopping %v queue(s)", len(s.jobQueues))
	for _, q := range s.jobQueues {
		...
		if q.running {
			q.stop <- true
			<-q.stopped
			klog.V(5).Infof("Stopped queue %v", q.interval)
			q.running = false
		}
	}
}
...
func (s *Scheduler) enqueueOnce(check check.Check) {
	klog.Infof("Scheduling check %v for one-time execution", check)
	s.wgOneTime.Add(1)
