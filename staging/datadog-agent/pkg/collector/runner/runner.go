...
import (
	...
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/collector/scheduler"
...
func NewRunner() *Runner {
	...
	klog.Infof("Runner started with %d workers.", numWorkers)
	return r
}
...
func (r *Runner) UpdateNumWorkers(numChecks int64) {
	...
	if added > 0 {
		klog.Infof("Added %d workers to runner: now at "+runnerStats.Get("Workers").String()+" workers.", added)
	}
}
...
func (r *Runner) Stop() {
	if atomic.LoadUint32(&r.running) == 0 {
		klog.V(5).Info("Runner already stopped, nothing to do here...")
		return
	}

	klog.Info("Runner is shutting down...")
	...
	for _, c := range r.runningChecks {
		...
		go func(c check.Check) {
			klog.Infof("Stopping Check %v that is still running...", c)
			...
			case <-time.After(stopCheckTimeout):
				...
				klog.Warningf("Check %v not responding after %v", c, stopCheckTimeout)
			}
		}(c)
	}
	r.m.Unlock()
	...
	case <-time.After(stopAllChecksTimeout):
		...
		klog.Errorf("Some checks not responding after %v, timing out...", stopAllChecksTimeout)
	}
}
...
func (r *Runner) StopCheck(id check.ID) error {
	...
	if c, isRunning := r.runningChecks[id]; isRunning {
		klog.V(5).Infof("Stopping check %s", c.ID())
		go func() {
		...
		klog.V(5).Infof("Check %s is not running, not stopping it", id)
		return nil
	}
...
func (r *Runner) work() {
	klog.V(5).Info("Ready to process checks...")
	defer TestWg.Done()
	...
	for check := range r.pending {
		...
		if _, isRunning := r.runningChecks[check.ID()]; isRunning {
			klog.V(5).Infof("Check %s is already running, skip execution...", check)
			r.m.Unlock()
			continue
		} else {
		...
		if e != nil {
			klog.Errorf("Error getting default sender: %v. Not sending status check for %s", e, check)
		}
		...
		if err != nil {
			klog.Errorf("Error running check %s: %s", check, err)
		...
		if check.Interval() == 0 {
			klog.Infof("Check %v one-time's execution has finished", check)
			return
		}
	}

	klog.V(5).Info("Finished processing checks.")
}
...
func addWorkStats(c check.Check, execTime time.Duration, err error, warnings []error, mStats map[string]int64) {
	...
	checkStats.M.Lock()
	klog.V(6).Infof("Add stats for %s", string(c.ID()))
...
func RemoveCheckStats(checkID check.ID) {
	...
	klog.V(5).Infof("Remove stats for %s", string(checkID))
