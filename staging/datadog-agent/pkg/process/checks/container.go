...
import (
	...
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/process/statsd"
	...
	containercollectors "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containers/collectors"
...
func (c *ContainerCheck) Run(cfg *config.AgentConfig, groupID int32) ([]model.MessageBody, error) {
	...
	if err == containercollectors.ErrPermaFail || err == containercollectors.ErrNothingYet {
		if c.containerFailedLogLimit.ShouldLog() {
			klog.V(5).Info("container collector was not detected, container check will not return any data. This message will logged for the first ten occurrences, and then every ten minutes")
		}
		return nil, nil
	}
	...
	if len(ctrList) == 0 {
		klog.V(6).Info("no containers found")
		return nil, nil
	}
...
func fmtContainers(ctrList []*containers.Container, lastRates map[string]util.ContainerRateMetrics, lastRun time.Time) []*model.Container {
	...
	for _, ctr := range ctrList {
		...
		if err != nil {
			klog.Errorf("unable to retrieve tags for container: %s", err)
			tags = []string{}
		}
		...
		if ctr.Type == containers.RuntimeNameGarden && len(tags) == 0 {
			...
			klog.V(5).Infof("No tags found for app %s, it has probably not been discovered by the DCA, skipping.", ctr.ID)
			continue
		}
