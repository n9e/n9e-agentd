	...
	for _, detector := range detectors {
		...
		if hit {
			...
			if ok {
				...
				if err != nil {
					klog.V(5).Infof("Cannot update container metrics via %s: %s", name, err)
					continue
				}
				klog.V(6).Infof("Got %d containers from cache", len(containers))
				result = append(result, containers...)
				continue
			}
			klog.Errorf("Invalid container list cache format, forcing a cache miss")
		}

		// If cache empty/expired, get a new container list
		containers, err := l.List()
		if err != nil {
			klog.Errorf("Cannot list containers via %s: %s", name, err)
			continue
		}
		cache.Cache.Set(cacheKey, containers, containerCacheDuration)
		klog.V(6).Infof("Got %d containers from source %s", len(containers), name)
		result = append(result, containers...)
	}
