...
import (
	...
	oconfig "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/orchestrator/config"
	...
	ddgrpc "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/grpc"
...
func (a AgentConfig) CheckInterval(checkName string) time.Duration {
	...
	if !ok {
		klog.Errorf("missing check interval for '%s', you must set a default", checkName)
		d = 10 * time.Second
	}
	return d
}
...
func loadConfigIfExists(path string) error {
	if util.PathExists(path) {
		...
		klog.Infof("no config exists at %s, ignoring...", path)
	}
	return nil
}
...
func NewAgentConfig(loggerName config.LoggerName, yamlPath, netYamlPath string) (*AgentConfig, error) {
	...
	if err := setupLogger(loggerName, cfg.LogFile, cfg); err != nil {
		klog.Errorf("failed to setup configured logger: %s", err)
		return nil, err
	}
	...
	if syscfg.Enabled {
		...
		if !cfg.Enabled {
			klog.Info("enabling process-agent for connections check as the system-probe is enabled")
			cfg.Enabled = true
		}
	}
	...
	if cfg.proxy, err = proxyFromEnv(cfg.proxy); err != nil {
		klog.Errorf("error parsing environment proxy settings, not using a proxy: %s", err)
		cfg.proxy = nil
	}
	...
	if err := validate.ValidHostname(cfg.HostName); err != nil {
		...
		if hostname, err := getHostname(cfg.DDAgentBin, cfg.grpcConnectionTimeout); err == nil {
			...
			klog.Errorf("Cannot get hostname: %v", err)
		}
	}
	...
	if cfg.Windows.ArgsRefreshInterval == 0 {
		klog.Warningf("invalid configuration: windows_collect_skip_new_args was set to 0.  Disabling argument collection")
	...
	if cfg.Orchestrator.OrchestrationCollectionEnabled {
		if cfg.Orchestrator.KubeClusterName != "" {
			...
			klog.Warningf("Failed to auto-detect a Kubernetes cluster name. Pod collection will not start. To fix this, set it manually via the cluster_name config option")
		}
	}
...
func loadEnvVariables() {
	...
	if apiKey != "" { // We don't want to overwrite the API KEY provided as an environment variable
		klog.Infof("overriding API key from env %s value", envKey)
	...
	if v := os.Getenv("DD_PROCESS_ADDITIONAL_ENDPOINTS"); v != "" {
		...
		if err := json.Unmarshal([]byte(v), &endpoints); err != nil {
			klog.Errorf(`Could not parse DD_PROCESS_ADDITIONAL_ENDPOINTS: %v. It must be of the form '{"https://process.agent.datadoghq.com": ["apikey1", ...], ...}'.`, err)
		} else {
	...
	if v := os.Getenv("DD_ORCHESTRATOR_ADDITIONAL_ENDPOINTS"); v != "" {
		endpoints := make(map[string][]string)
		if err := json.Unmarshal([]byte(v), &endpoints); err != nil {
			klog.Errorf(`Could not parse DD_ORCHESTRATOR_ADDITIONAL_ENDPOINTS: %v. It must be of the form '{"https://process.agent.datadoghq.com": ["apikey1", ...], ...}'.`, err)
		} else {
...
func getHostname(ddAgentBin string, grpcConnectionTimeout time.Duration) (string, error) {
	...
	if fargate.IsFargateInstance() {
		...
		klog.Errorf("failed to get Fargate host: %v", err)
	}
	...
	klog.Errorf("failed to get hostname from grpc: %v", err)
	...
	klog.Errorf("failed to get hostname from cmd: %v", err)
...
func getHostnameFromGRPC(grpcClientFn func(ctx context.Context, opts ...grpc.DialOption) (pb.AgentClient, error), grpcConnectionTimeout time.Duration) (string, error) {
	...
	klog.V(5).Infof("retrieved hostname:%s from datadog agent via grpc", reply.Hostname)
