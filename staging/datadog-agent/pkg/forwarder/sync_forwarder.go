...
func (f *SyncForwarder) SubmitV1Series(payload Payloads, extra http.Header) error {
	transactions := f.defaultForwarder.createHTTPTransactions(n9eV1SeriesEndpoint, payload, true, extra)
...
func (f *SyncForwarder) SubmitSeries(payload Payloads, extra http.Header) error {
	transactions := f.defaultForwarder.createHTTPTransactions(n9eSeriesEndpoint, payload, false, extra)
	return f.sendHTTPTransactions(transactions)
}
