...
func (sl SketchSeriesList) MarshalSplitCompress(bufferContext *marshaler.BufferContext) ([]*[]byte, error) {
	...
	dsl := make([]agentpayload.SketchPayload_Sketch_Dogsketch, 1)
	for _, ss := range sl {
		if len(ss.Points) > cap(dsl) {
			dsl = make([]agentpayload.SketchPayload_Sketch_Dogsketch, len(ss.Points))
		}

		for i, p := range ss.Points {
			b := p.Sketch.Basic
			k, n := p.Sketch.Cols()
			dsl[i] = agentpayload.SketchPayload_Sketch_Dogsketch{
				Ts:  p.Ts,
				Cnt: b.Cnt,
				Min: b.Min,
				Max: b.Max,
...
func (sl SketchSeriesList) Marshal() ([]byte, error) {
	pb := &agentpayload.SketchPayload{
		Sketches: make([]agentpayload.SketchPayload_Sketch, 0, len(sl)),
	}

	for _, ss := range sl {
		dsl := make([]agentpayload.SketchPayload_Sketch_Dogsketch, 0, len(ss.Points))

		for _, p := range ss.Points {
			b := p.Sketch.Basic
			k, n := p.Sketch.Cols()
			dsl = append(dsl, agentpayload.SketchPayload_Sketch_Dogsketch{
				Ts:  p.Ts,
				Cnt: b.Cnt,
				Min: b.Min,
				Max: b.Max,
