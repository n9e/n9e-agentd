	...
	for {
		...
		case err := <-errs:
			if err != nil && err != io.EOF {
				klog.Errorf("stopping collection: %s", err)
				return err
			}
			return nil
		}
...
func (c *DockerCollector) processEvent(e *docker.ContainerEvent) {
	...
	case "start":
		...
		if err != nil {
			klog.V(5).Infof("Error fetching tags for container '%s': %v", e.ContainerName, err)
		}
		info = &TagInfo{
...
func (c *DockerCollector) fetchForDockerID(cID string) ([]string, []string, []string, []string, error) {
	...
	if err != nil {
		if !errors.IsNotFound(err) {
			klog.V(5).Infof("Failed to inspect container %s - %s", cID, err)
		}
