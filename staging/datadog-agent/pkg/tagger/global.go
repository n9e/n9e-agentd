...
func Init() {
	initOnce.Do(func() {
		...
		if err != nil {
			klog.Warningf("failed to parse check tag cardinality, defaulting to low. Error: %s", err)
		...
		if config.C.IsCLCRunner() {
			klog.Infof("Tagger not started on CLC")
			return
		}

		err = defaultTagger.Init()
		if err != nil {
			klog.Errorf("failed to start the tagger: %s", err)
		}
	})
}
