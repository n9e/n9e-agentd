...
func NewSocketFilterSnooper(cfg *config.Config, source PacketSource) (*SocketFilterSnooper, error) {
	...
	if cfg.CollectDNSStats {
		...
		klog.Infof("DNS Stats Collection has been enabled. Maximum number of stats objects: %d", cfg.MaxDNSStats)
		if cfg.CollectDNSDomains {
			klog.Infof("DNS domain collection has been enabled")
		}
	} else {
		klog.Infof("DNS Stats Collection has been disabled.")
	}
...
func (s *SocketFilterSnooper) processPacket(data []byte, ts time.Time) error {
	...
	if err := s.parser.ParseInto(data, t, &pktInfo); err != nil {
		...
		default:
			...
			klog.V(6).Infof("error decoding DNS payload: %v", err)
		}
		return nil
	}
...
func (s *SocketFilterSnooper) pollPackets() {
	for {
		...
		if err != nil {
			klog.Warningf("error reading packet: %s", err)
		}
...
func (s *SocketFilterSnooper) logDNSStats() {
	...
	for {
		...
		case <-ticker.C:
			...
			klog.Infof("DNS Stats. Queries :%d, Successes :%d, Errors: %d", queries, successes, errors)
		case <-s.exit:
			return
		}
	}
