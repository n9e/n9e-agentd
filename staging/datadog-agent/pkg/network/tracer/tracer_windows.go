...
func NewTracer(config *config.Config) (*Tracer, error) {
	...
	if err != nil && errors.Cause(err) == syscall.Errno(syscall.ERROR_FILE_NOT_FOUND) {
		klog.V(5).Infof("could not create driver interface: %v", err)
...
func (t *Tracer) Stop() {
	...
	if err != nil {
		klog.Errorf("error closing driver interface: %s", err)
	}
}
...
func (t *Tracer) GetActiveConnections(clientID string) (*network.Connections, error) {
	...
	if err != nil {
		klog.Errorf("failed to get connections")
		return nil, err
	}
...
func (t *Tracer) GetStats() (map[string]interface{}, error) {
	...
	if err != nil {
		klog.Errorf("not printing driver stats: %v", err)
	}
