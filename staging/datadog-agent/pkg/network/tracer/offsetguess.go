...
func guessOffsets(m *manager.Manager, cfg *config.Config) ([]manager.ConstantEditor, error) {
	...
	klog.V(5).Infof("Checking for offsets with threshold of %d", threshold)
...
func logAndAdvance(status *tracerStatus, offset C.__u64, next C.__u64) {
	...
	if offset != notApplicable {
		klog.V(5).Infof("Successfully guessed %v with offset of %d bytes", whatString[guess], offset)
	} else {
		klog.V(5).Infof("Could not guess offset for %v", whatString[guess])
	}
	if next != notApplicable {
		klog.V(5).Infof("Started offset guessing for %v", whatString[next])
		status.what = next
	}
}
