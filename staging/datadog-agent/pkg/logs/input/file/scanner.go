...
func (s *Scanner) startNewTailer(file *File, m config.TailingMode) bool {
	...
	if err != nil {
		klog.Warningf("Could not recover offset for file with path %v: %v", file.Path, err)
	}

	klog.Infof("Starting a new tailer for: %s (offset: %d, whence: %d) for tailer key %s", file.Path, offset, whence, file.GetScanKey())
...
func (s *Scanner) createTailer(file *File, outputChan chan *message.Message) *Tailer {
	return NewTailer(outputChan, file, s.tailerSleepDuration)
}
