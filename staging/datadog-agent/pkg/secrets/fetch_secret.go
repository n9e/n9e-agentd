...
func execCommand(inputPayload string) ([]byte, error) {
	...
	klog.V(5).Infof("secret_backend_command '%s' completed in %s", secretBackendCommand, elapsed)

	if err != nil {
		klog.Errorf("secret_backend_command stderr: %s", stderr.buf.String())
...
func fetchSecret(secretsHandle []string, origin string) (map[string]string, error) {
	...
	klog.V(5).Infof("calling secret_backend_command with payload: '%s'", jsonPayload)
