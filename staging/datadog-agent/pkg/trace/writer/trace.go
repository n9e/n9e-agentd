...
func NewTraceWriter(cfg *config.AgentConfig) *TraceWriter {
	...
	klog.V(5).Infof("Trace writer initialized (climit=%d qsize=%d)", climit, qsize)
...
func (w *TraceWriter) Stop() {
	klog.V(5).Info("Exiting trace writer. Trying to flush whatever is left...")
	w.stop <- struct{}{}
	<-w.stop
...
func (w *TraceWriter) addSpans(pkg *SampledSpans) {
	...
	if len(pkg.Traces) > 0 {
		klog.V(6).Infof("Handling new trace with %d spans: %v", pkg.SpanCount, pkg.Traces)
		w.traces = append(w.traces, pkg.Traces...)
	}
	if len(pkg.Events) > 0 {
		klog.V(6).Infof("Handling new package with %d events: %v", len(pkg.Events), pkg.Events)
...
func (w *TraceWriter) flush() {
	...
	if err != nil {
		klog.Errorf("Failed to serialize payload, data dropped: %v", err)
		return
	}
	...
	go func() {
		...
		if err != nil {
			...
			klog.Errorf("gzip.NewWriterLevel: %d", err)
			return
		}
		if _, err := gzipw.Write(b); err != nil {
			klog.Errorf("Error gzipping trace payload: %v", err)
		}
		if err := gzipw.Close(); err != nil {
			klog.Errorf("Error closing gzip stream when writing trace payload: %v", err)
		}
...
func (w *TraceWriter) recordEvent(t eventType, data *eventData) {
	...
	switch t {
	case eventTypeRetry:
		klog.V(5).Infof("Retrying to flush trace payload; error: %s", data.err)
	...
	case eventTypeRejected:
		klog.Warningf("Trace writer payload rejected by edge: %v", data.err)
