...
func (p *Pod) Owners() []PodOwner {
	...
	// Error handling
	if err != nil {
		klog.V(5).Infof("Cannot parse created-by field for pod %q: %s", p.Metadata.Name, err)
		return nil
	}
	if ref.Kind != "SerializedReference" {
		klog.V(5).Infof("Cannot parse created-by field for pod %q: unknown kind %q", p.Metadata.Name, ref.Kind)
		return nil
	}
