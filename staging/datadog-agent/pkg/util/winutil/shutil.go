...
func GetProgramDataDirForProduct(product string) (path string, err error) {
	...
	if err != nil {
		...
		klog.V(5).Infof("Windows installation key root (%s) not found, using default program data dir", keyname)
		return getDefaultProgramDataDir()
	}
	defer k.Close()
	val, _, err := k.GetStringValue("ConfigRoot")
	if err != nil {
		klog.Warningf("Windows installation key config not found, using default program data dir")
		return getDefaultProgramDataDir()
	}
	path = val
	return
