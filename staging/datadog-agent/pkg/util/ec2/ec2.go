...
func GetInstanceID() (string, error) {
	if !config.C.IsCloudProviderEnabled(CloudProviderName) {
		return "", fmt.Errorf("cloud provider is disabled by configuration")
	}

	instanceID, err := getMetadataItemWithMaxLength("/instance-id",
		config.C.MetadataEndpointsMaxHostnameSize)
	if err != nil {
		if instanceID, found := cache.Cache.Get(instanceIDCacheKey); found {
			klog.V(5).Infof("Unable to get ec2 instanceID from aws metadata, returning cached instanceID '%s': %s", instanceID, err)
...
func GetHostname() (string, error) {
	...
	hostname, err := getMetadataItemWithMaxLength("/hostname",
		config.C.MetadataEndpointsMaxHostnameSize)
	if err != nil {
		if hostname, found := cache.Cache.Get(hostnameCacheKey); found {
			klog.V(5).Infof("Unable to get ec2 hostname from aws metadata, returning cached hostname '%s': %s", hostname, err)
...
func getMetadataItem(endpoint string) (string, error) {
	res, err := doHTTPRequest(metadataURL+endpoint, http.MethodGet, map[string]string{},
		config.C.EC2PreferImdsv2)
	if err != nil {
...
func doHTTPRequest(url string, method string, headers map[string]string, useToken bool) (*http.Response, error) {
	...
	if useToken {
		token, err := getToken()
		if err != nil {
			klog.Warningf("ec2_prefer_imdsv2 is set to true in the configuration but the agent was unable to proceed: %s", err)
		} else {
