...
import (
	...
	dderrors "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/errors"
...
func ConnectToDocker(ctx context.Context) (*client.Client, error) {
	...
	klog.V(5).Infof("Successfully connected to Docker server")
...
func (d *DockerUtil) ResolveImageName(image string) (string, error) {
	...
	if _, ok := d.imageNameBySha[image]; !ok {
		...
		if len(r.RepoTags) > 0 {
			...
			klog.V(5).Infof("No information in image/inspect to resolve: %s", image)
			d.imageNameBySha[image] = image
		}
	}
...
func (d *DockerUtil) Inspect(id string, withSize bool) (types.ContainerJSON, error) {
	...
	if hit {
		...
		if !ok {
			klog.Errorf("Invalid inspect cache format, forcing a cache miss")
		} else {
