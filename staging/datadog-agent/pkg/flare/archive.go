...
var (
	//pprofURL     = fmt.Sprintf("http://127.0.0.1:%v/debug/pprof/goroutine?debug=2", config.C.Telemetry.Port)
	//telemetryURL = fmt.Sprintf("http://127.0.0.1:%v/telemetry", config.C.Telemetry.Port)
...
func createArchive(confSearchPaths SearchPaths, local bool, zipFilePath string, logFilePaths []string, pdata ProfileData) (string, error) {
	...
	if _, err = os.Stat(config.C.AuthTokenFile); err == nil && !os.IsNotExist(err) {
		permsInfos.add(config.C.AuthTokenFile)
	}
...
func zipExpVar(tempDir, hostname string) error {
	...
	apmPort := "8126"
	if v := config.C.Apm.ReceiverPort; v > 0 {
		apmPort = strconv.Itoa(v)
	}
...
func zipTaggerList(tempDir, hostname string) error {
	...
	var r []byte
	err = common.Client.ApiCall("GET", "/api/v1/tagger-list", nil, nil, &r)
	if err != nil {
		return err
	}
...
func zipTelemetry(tempDir, hostname string) error {
	return zipHTTPCallContent(tempDir, hostname, "telemetry.log", "/metrics")
}

func zipStackTraces(tempDir, hostname string) error {
	return zipHTTPCallContent(tempDir, hostname, routineDumpFilename, "/debug/pprof/goroutine?debug=2")
}
...
func zipHTTPCallContent(tempDir, hostname, filename, url string) error {
	...
	return common.Client.ApiCall("GET", url, nil, nil, w,
		cmdcli.WithClient(common.Client.GetClient("telemetry")),
		cmdcli.WithTimeout(4*time.Second))
}
