...
import (
	...
	sprobe "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/security/probe"
...
func (a *APIServer) SendEvent(rule *rules.Rule, event Event) {
	...
	if err != nil {
		klog.Error(errors.Wrap(err, "failed to marshal event"))
		return
	}

	ruleEventJSON, err := json.Marshal(ruleEvent)
	if err != nil {
		klog.Error(errors.Wrap(err, "failed to marshal event context"))
		return
	}

	data := append(probeJSON[:len(probeJSON)-1], ',')
	data = append(data, ruleEventJSON[1:]...)
	klog.V(6).Infof("Sending event message for rule `%s` to security-agent `%s`", rule.ID, string(data))
...
func (a *APIServer) expireEvent(msg *api.SecurityEventMessage) {
	...
	klog.V(6).Infof("the event server channel is full, an event of ID %v was dropped", msg.RuleID)
}
