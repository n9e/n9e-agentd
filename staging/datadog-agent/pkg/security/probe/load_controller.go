...
func (lc *LoadController) discardNoisiestProcess() {
	...
	klog.V(6).Infof("discarding %s events from pid %d for %s seconds", maxKey.Event, maxKey.Pid, lc.DiscarderTimeout)
	...
	if err := lc.probe.pidDiscarders.discardWithTimeout(maxKey.Event, maxKey.Pid, timeout); err != nil {
		klog.Warningf("couldn't insert temporary discarder: %v", err)
		return
	}
	...
	if lc.statsdClient != nil {
		...
		if err := lc.statsdClient.Count(metrics.MetricLoadControllerPidDiscarder, 1, tags, 1.0); err != nil {
			klog.Warningf("couldn't send load_controller.pids_discarder metric: %v", err)
			return
		}
		...
		if process == nil {
			klog.Warningf("Unable to resolver process with pid: %d", maxKey.Pid)
			return
		}
