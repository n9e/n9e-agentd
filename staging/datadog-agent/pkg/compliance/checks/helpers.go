...
import (
	...
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/jsonquery"
	"k8s.io/klog/v2"
...
func evalGoTemplate(s string, obj interface{}) string {
	...
	if err != nil {
		klog.Warningf("failed to parse template %q: %v", s, err)
		return ""
	}

	b := &strings.Builder{}
	if err := tmpl.Execute(b, obj); err != nil {
		klog.V(6).Infof("failed to execute template %q: %v", s, err)
		return ""
	}
	return b.String()
}
