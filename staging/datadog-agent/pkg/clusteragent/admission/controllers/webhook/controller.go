...
func (c *Controller) Run(stopCh <-chan struct{}) {
	...
	klog.Infof("Starting webhook controller for secret %s/%s and webhook %s", c.config.GetSecretNs(), c.config.GetSecretName(), c.config.GetName())
	defer klog.Infof("Stopping webhook controller for secret %s/%s and webhook %s", c.config.GetSecretNs(), c.config.GetSecretName(), c.config.GetName())
...
func (c *Controller) handleSecretUpdate(oldObj, newObj interface{}) {
	...
	if !ok {
		klog.V(5).Infof("Expected Secret object, got: %v", newObj)
		return
	}
	oldSecret, ok := oldObj.(*corev1.Secret)
	if !ok {
		klog.V(5).Infof("Expected Secret object, got: %v", oldObj)
		return
	}
...
func (c *Controller) handleWebhookUpdate(oldObj, newObj interface{}) {
	...
	if !ok {
		klog.V(5).Infof("Expected MutatingWebhookConfiguration object, got: %v", newObj)
		return
	}
	oldWebhook, ok := oldObj.(*admiv1beta1.MutatingWebhookConfiguration)
	if !ok {
		klog.V(5).Infof("Expected MutatingWebhookConfiguration object, got: %v", oldObj)
		return
	}
...
func (c *Controller) enqueue(obj interface{}) {
	...
	if err != nil {
		klog.V(5).Infof("Couldn't get key for object %v: %v, adding it to the queue with an unnamed key", obj, err)
		c.queue.Add(struct{}{})
		return
	}
	klog.V(5).Infof("Adding object with key %s to the queue", key)
	c.queue.Add(key)
}
...
func (c *Controller) processNextWorkItem() bool {
	...
	if err := c.reconcile(); err != nil {
		c.requeue(key)
		klog.Infof("Couldn't reconcile Webhook %s: %v", c.config.GetName(), err)
		metrics.ReconcileErrors.Inc(metrics.WebhooksControllerName)
		return true
	}

	c.queue.Forget(key)
	klog.V(5).Infof("Webhook %s reconciled successfully", c.config.GetName())
...
func (c *Controller) reconcile() error {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			klog.Infof("Webhook %s was not found, creating it", c.config.GetName())
			return c.createWebhook(secret)
		}
		return err
	}

	klog.V(5).Infof("The Webhook %s was found, updating it", c.config.GetName())
...
func (c *Controller) createWebhook(secret *corev1.Secret) error {
	...
	if errors.IsAlreadyExists(err) {
		klog.Infof("Webhook %s already exists", webhook.GetName())
		return nil
	}
	return err
}
