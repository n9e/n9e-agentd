...
func (agg *BufferedAggregator) AddAgentStartupTelemetry(agentVersion string) {
	...
	metric := &metrics.MetricSample{
		Name:       fmt.Sprintf("%s.started", agg.agentName),
		Value:      1,
		Tags:       agg.tags(true),
		Host:       agg.hostname,
...
func (agg *BufferedAggregator) handleSenderSample(ss senderMetricSample) {
	...
	if checkSampler, ok := agg.checkSamplers[ss.id]; ok {
		...
		klog.V(5).Infof("CheckSampler with ID '%s' doesn't exist, can't handle senderMetricSample", ss.id)
	}
}
...
func (agg *BufferedAggregator) handleSenderBucket(checkBucket senderHistogramBucket) {
	...
	if checkSampler, ok := agg.checkSamplers[checkBucket.id]; ok {
		...
		klog.V(5).Infof("CheckSampler with ID '%s' doesn't exist, can't handle histogram bucket", checkBucket.id)
	}
}
...
func (agg *BufferedAggregator) pushSketches(start time.Time, sketches metrics.SketchSeriesList) {
	klog.V(5).Infof("Flushing %d sketches to the forwarder", len(sketches))
	err := agg.serializer.SendSketch(sketches)
	state := stateOk
	if err != nil {
		klog.Warningf("Error flushing sketch: %v", err)
...
func (agg *BufferedAggregator) pushSeries(start time.Time, series metrics.Series) {
	klog.V(5).Infof("Flushing %d series to the forwarder", len(series))
	err := agg.serializer.SendSeries(series)
	state := stateOk
	if err != nil {
		klog.Warningf("Error flushing series: %v", err)
...
func (agg *BufferedAggregator) sendSeries(start time.Time, series metrics.Series, waitForSerializer bool) {
	...
	series = append(series, &metrics.Serie{
		Name:           fmt.Sprintf("%s.running", agg.agentName),
	...
	series = append(series, &metrics.Serie{
		Name:           fmt.Sprintf("noindex.%s.payload.dropped", agg.agentName),
	...
	if config.C.LogPayloads {
		klog.V(5).Info("Flushing the following metrics:")
		for _, serie := range series {
			klog.V(5).Infof("%s", serie)
		}
	}
...
func (agg *BufferedAggregator) sendServiceChecks(start time.Time, serviceChecks metrics.ServiceChecks) {
	klog.V(5).Infof("Flushing %d service checks to the forwarder", len(serviceChecks))
	state := stateOk
	if err := agg.serializer.SendServiceChecks(serviceChecks); err != nil {
		klog.Warningf("Error flushing service checks: %v", err)
...
func (agg *BufferedAggregator) flushServiceChecks(start time.Time, waitForSerializer bool) {
	...
	if config.C.LogPayloads {
		klog.V(5).Info("Flushing the following Service Checks:")
...
func (agg *BufferedAggregator) sendEvents(start time.Time, events metrics.Events) {
	klog.V(5).Infof("Flushing %d events to the forwarder", len(events))
	err := agg.serializer.SendEvents(events)
	state := stateOk
	if err != nil {
		klog.Warningf("Error flushing events: %v", err)
...
func (agg *BufferedAggregator) flushEvents(start time.Time, waitForSerializer bool) {
	...
	if config.C.LogPayloads {
		klog.V(5).Info("Flushing the following Events:")
		for _, event := range events {
			klog.V(5).Infof("%s", event)
		}
	}
...
func (agg *BufferedAggregator) Stop() {
	...
	timeout := config.C.AggregatorStopTimeout
	if timeout > 0 {
		...
		case <-time.After(timeout):
			klog.Errorf("flushing data after stop timed out")
		}
	}
...
func (agg *BufferedAggregator) run() {
	if agg.TickerChan == nil {
		if agg.flushInterval != 0 {
			...
			klog.V(5).Infof("aggregator flushInterval set to 0: aggregator won't flush data")
		}
	}
	...
	for {
		...
		case checkHistogramBucket := <-agg.checkHistogramBucketIn:
			klog.V(5).Infof("agg.checkHistogramBucketIn")
		...
		case ms := <-agg.bufferedMetricInWithTs:
			klog.V(5).Infof("agg.bufferedMetricInWithTs")
		...
		case ms := <-agg.bufferedMetricIn:
			klog.V(5).Infof("agg.bufferedMetricIn")
			aggregatorDogstatsdMetricSample.Add(int64(len(ms)))
			tlmProcessed.Add(float64(len(ms)), "dogstatsd_metrics")
			for i := 0; i < len(ms); i++ {
		...
		case serviceChecks := <-agg.bufferedServiceCheckIn:
			klog.V(5).Infof("agg.bufferedServiceCheckIn")
		...
		case events := <-agg.bufferedEventIn:
			klog.V(5).Infof("agg.bufferedEventIn")
		...
		case h := <-agg.hostnameUpdate:
			klog.V(5).Infof("agg.hostnameUpdate")
		...
		case orchestratorMetadata := <-agg.orchestratorMetadataIn:
			klog.V(5).Infof("agg.orchestratorMetadataIn")
			...
			go func(orchestratorMetadata senderOrchestratorMetadata) {
				...
				if err != nil {
					...
					klog.Errorf("Error submitting orchestrator data: %s", err)
				}
...
func (agg *BufferedAggregator) tags(withVersion bool) []string {
	...
	if agg.tlmContainerTagsEnabled {
		...
		if err != nil {
			klog.V(5).Infof("Couldn't get Agent tags: %v", err)
		}
	}
	if withVersion {
