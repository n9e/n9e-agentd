...
import (
	...
	traceAgent "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/trace/agent"
	"k8s.io/klog/v2"
)
...
func (d *Daemon) SetFlushStrategy(strategy flush.Strategy) {
	klog.V(5).Infof("Set flush strategy: %s (was: %s)", strategy.String(), d.flushStrategy.String())
...
func (d *Daemon) TriggerFlush(ctx context.Context, shutdown bool) {
	...
	if d.useAdaptiveFlush && !shutdown {
		...
		if newStrat.String() != d.flushStrategy.String() {
			klog.V(5).Info("Switching to flush strategy:", newStrat)
...
func StartDaemon(stopCh chan struct{}) *Daemon {
	klog.V(5).Info("Starting daemon to receive messages from runtime...")
	...
	klog.V(5).Info("Adaptive flush is enabled")
...
func (d *Daemon) EnableLogsCollection() (string, chan aws.LogMessage, error) {
	...
	klog.V(5).Infof("Logs collection route has been initialized. Logs must be sent to %s", httpAddr)
...
func (l *LogsCollection) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.Unmarshal(data, &messages); err != nil {
		klog.Error("Can't read log message")
		w.WriteHeader(400)
	} else {
		...
		for _, message := range messages {
			...
			case aws.LogTypePlatformLogsDropped:
				klog.V(5).Info("Logs were dropped by the AWS Lambda Logs API")
			}
...
func (h *Hello) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	klog.V(5).Info("Hit on the serverless.Hello route.")
	// if the DogStatsD daemon isn't ready, wait for it.
...
func (f *Flush) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	klog.V(5).Info("Hit on the serverless.Flush route.")

	if !f.daemon.flushStrategy.ShouldFlush(flush.Stopping, time.Now()) {
		klog.V(5).Info("The flush strategy", f.daemon.flushStrategy, " has decided to not flush in moment:", flush.Stopping)
		return
	}

	klog.V(5).Info("The flush strategy", f.daemon.flushStrategy, " has decided to flush in moment:", flush.Stopping)
