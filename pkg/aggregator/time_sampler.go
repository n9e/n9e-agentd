...
func (s *TimeSampler) addSample(metricSample *metrics.MetricSample, timestamp float64) {
	...
	default:
		...
		if err := bucketMetrics.AddSample(contextKey, metricSample, timestamp, s.interval); err != nil {
			klog.V(5).Info("Ignoring sample '%s' on host '%s' and tags '%s': %s", metricSample.Name, metricSample.Host, metricSample.Tags, err)
		}
	}
}
...
func (s *TimeSampler) flushSeries(cutoffTime int64) metrics.Series {
	...
	for _, serie := range rawSeries {
		...
		if existingSerie, ok := serieBySignature[serieSignature]; ok {
			...
			if !ok {
				klog.Errorf("Ignoring all metrics on context key '%v': inconsistent context resolver state: the context is not tracked", serie.ContextKey)
				continue
			}
...
func (s *TimeSampler) flush(timestamp float64) (metrics.Series, metrics.SketchSeriesList) {
	...
	// expiring contexts
	s.contextResolver.expireContexts(timestamp - float64(config.C.Statsd.ContextExpirySeconds)/float64(time.Second))
...
func (s *TimeSampler) flushContextMetrics(timestamp int64, contextMetrics metrics.ContextMetrics) []*metrics.Serie {
	...
	for ckey, err := range errors {
		...
		if !ok {
			klog.Errorf("Can't resolve context of error '%s': inconsistent context resolver state: context with key '%v' is not tracked", err, ckey)
			continue
		}
		klog.Infof("No value returned for dogstatsd metric '%s' on host '%s' and tags '%s': %s", context.Name, context.Host, context.Tags, err)
	}
	return series
}
...
func (s *TimeSampler) countersSampleZeroValue(timestamp int64, contextMetrics metrics.ContextMetrics, counterContextsToDelete map[ckey.ContextKey]struct{}) {
	expirySeconds := float64(config.C.Statsd.ExpirySeconds) / float64(time.Second)
	for counterContext, lastSampled := range s.counterLastSampledByContext {
		if expirySeconds+lastSampled > float64(timestamp) {
			...
			if err != nil {
				klog.Errorf("Error updating context: %s", err)
			}
		} else {
