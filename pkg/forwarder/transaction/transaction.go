...
var Trace = &httptrace.ClientTrace{
	DNSDone: func(dnsInfo httptrace.DNSDoneInfo) {
		if dnsInfo.Err != nil {
			...
			klog.V(5).Infof("DNS Lookup failure: %s", dnsInfo.Err)
			return
		}
		connectionDNSSuccess.Add(1)
		tlmConnectEvents.Inc("dns_lookup_success")
		klog.V(6).Infof("DNS Lookup success, addresses: %s", dnsInfo.Addrs)
	},
	WroteRequest: func(wroteInfo httptrace.WroteRequestInfo) {
		if wroteInfo.Err != nil {
			...
			klog.V(5).Infof("Request writing failure: %s", wroteInfo.Err)
		}
	},
	ConnectDone: func(network, addr string, err error) {
		if err != nil {
			...
			klog.V(5).Infof("Connection failure: %s", err)
			return
		}
		connectionConnectSuccess.Add(1)
		tlmConnectEvents.Inc("connection_success")
		klog.V(6).Infof("New successful connection to address: %q", addr)
	},
...
func (t *HTTPTransaction) internalProcess(ctx context.Context, client *http.Client) (int, []byte, error) {
	...
	if err != nil {
		klog.Errorf("Fail to read the response Body: %s", err)
		return 0, nil, err
	}
	...
	if transactionsSuccess.Value() == 1 {
		klog.Infof("Successfully posted payload to %q, the agent will only log transaction success every %d transactions", logURL, loggingFrequency)
		klog.V(6).Infof("Url: %q payload: %s", logURL, string(body))
		return resp.StatusCode, body, nil
	}
	if transactionsSuccess.Value()%loggingFrequency == 0 {
		klog.Infof("Successfully posted payload to %q", logURL)
		klog.V(6).Infof("Url: %q payload: %s", logURL, string(body))
		return resp.StatusCode, body, nil
	}
	klog.V(6).Infof("Successfully posted payload to %q: %s", logURL, string(body))
	return resp.StatusCode, body, nil
}
...
func (t *HTTPTransaction) SerializeTo(serializer TransactionsSerializer) error {
	...
	klog.V(6).Info("The transaction is not stored on disk because `storableOnDisk` is false.")
	return nil
}
