...
func (f *domainForwarder) retryTransactions(retryBefore time.Time) {
	...
	if !atomic.CompareAndSwapInt32(&f.isRetrying, 0, 1) {
		klog.Errorf("The forwarder is still retrying Transaction: this should never happens, you might want to lower the 'forwarder_retry_queue_payloads_max_size'")
		return
	}
	...
	if err != nil {
		klog.Errorf("Error when getting transactions from the retry queue", err)
	}
...
func (f *domainForwarder) addToTransactionRetryQueue(t transaction.Transaction) int {
	...
	if err != nil {
		klog.Errorf("Error when adding a transaction to the retry queue: %v", err)
	}
...
func (f *domainForwarder) Stop(purgeHighPrio bool) {
	...
	klog.Info("domainForwarder stopped")
