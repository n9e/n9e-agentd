...
func newBlockedEndpoints() *blockedEndpoints {
	...
	backoffFactor := cf.BackoffFactor
	if backoffFactor < 2 {
		klog.Warningf("Configured forwarder_backoff_factor (%v) is less than 2; 2 will be used", backoffFactor)
		backoffFactor = 2
	}

	backoffBase := cf.BackoffBase
	if backoffBase <= 0 {
		klog.Warningf("Configured forwarder_backoff_base (%v) is not positive; 2 will be used", backoffBase)
		backoffBase = 2
	}

	backoffMax := cf.BackoffMax
	if backoffMax <= 0 {
		klog.Warningf("Configured forwarder_backoff_max (%v) is not positive; 64 seconds will be used", backoffMax)
		backoffMax = 64
	}

	errorsMax := int(math.Floor(math.Log2(backoffMax/backoffBase))) + 1
	recInterval := cf.RecoveryInterval
	recoveryReset := cf.RecoveryReset
	if recoveryReset {
