...
import (
	...
	utilhttp "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/http"
	"k8s.io/klog/v2"
)
...
func (f *SyncForwarder) sendHTTPTransactions(transactions []*transaction.HTTPTransaction) error {
	for _, t := range transactions {
		if err := t.Process(context.Background(), f.client); err != nil {
			klog.Errorf("SyncForwarder.sendHTTPTransactions: %s", err)
		}
	}
	klog.V(5).Infof("SyncForwarder has flushed %d transactions", len(transactions))
	return nil
}
