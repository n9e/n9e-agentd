...
func (w *Worker) Stop(purgeHighPrio bool) {
	...
	L:
		for {
			select {
			case t := <-w.HighPrio:
				klog.V(5).Infof("Flushing one new transaction before stopping Worker")
...
func (w *Worker) process(ctx context.Context, t transaction.Transaction) {
	requeue := func() {
		...
		default:
			klog.Errorf("dropping transaction because the retry goroutine is too busy to handle another one")
		}
	}
	...
	if w.blockedList.isBlock(target) {
		requeue()
		klog.Errorf("Too many errors for endpoint '%s': retrying later", target)
	} else if err := t.Process(ctx, w.Client); err != nil {
		w.blockedList.close(target)
		requeue()
		klog.Errorf("Error while processing transaction: %v", err)
	} else {
...
func (w *Worker) resetConnections() {
	klog.V(5).Info("Resetting worker's connections")
