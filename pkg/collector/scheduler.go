...
import (
	...
	"github.com/n9e/n9e-agentd/pkg/collector/loaders"
...
func InitCheckScheduler(collector *Collector) *CheckScheduler {
	...
	for _, loader := range loaders.LoaderCatalog() {
		...
		klog.V(5).Infof("Added %s to Check Scheduler", loader)
	}
...
func (s *CheckScheduler) Schedule(configs []integration.Config) {
	...
	for _, c := range checks {
		...
		if err != nil {
			klog.Errorf("Unable to run Check %s: %v", c, err)
...
func (s *CheckScheduler) Unschedule(configs []integration.Config) {
	for _, config := range configs {
		...
		for _, id := range ids {
			...
			if err != nil {
				klog.Errorf("Error stopping check %s: %s", id, err)
...
func (s *CheckScheduler) AddLoader(loader check.Loader) {
	for _, l := range s.loaders {
		if l == loader {
			klog.Warningf("Loader %s was already added, skipping...", loader)
			return
		}
	}
...
func (s *CheckScheduler) getChecks(config integration.Config) ([]check.Check, error) {
	...
	for _, instance := range config.Instances {
		...
		if selectedInstanceLoader != "" {
			klog.V(5).Infof("Loading check instance for check '%s' using loader %s (init_config loader: %s, instance loader: %s)", config.Name, selectedInstanceLoader, initConfig.LoaderName, instanceConfig.LoaderName)
		} else {
			klog.V(5).Infof("Loading check instance for check '%s' using default loaders", config.Name)
		}
		...
		for _, loader := range s.loaders {
			...
			if (selectedInstanceLoader != "") && (selectedInstanceLoader != loader.Name()) {
				klog.V(5).Infof("Loader name %v does not match, skip loader %v for check %v", selectedInstanceLoader, loader.Name(), config.Name)
				continue
			}
			c, err := loader.Load(config, instance)
			if err == nil {
				klog.V(5).Infof("%v: successfully loaded check '%s'", loader, config.Name)
				...
				klog.V(5).Infof("%v: loading issue for JMX check '%s', the agent will still attempt to schedule it", loader, config.Name)
		...
		if len(errors) == numLoaders {
			klog.V(5).Infof("Unable to load a check from instance of config '%s': %s", config.Name, strings.Join(errors, "; "))
		}
	}
...
func (s *CheckScheduler) GetChecksFromConfigs(configs []integration.Config, populateCache bool) []check.Check {
	...
	for _, config := range configs {
		...
		if config.HasFilter(containers.MetricsFilter) {
			klog.V(5).Infof("Config %s is filtered out for metrics collection, ignoring it", config.Name)
			continue
		}
		configDigest := config.Digest()
		checks, err := s.getChecks(config)
		if err != nil {
			klog.Errorf("Unable to load the check: %v", err)
			continue
		}
		for _, c := range checks {
