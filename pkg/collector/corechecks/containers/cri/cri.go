...
import (
	...
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containers/cri"
	"k8s.io/klog/v2"
)
...
func (c *CRICheck) generateMetrics(sender aggregator.Sender, containerStats map[string]*pb.ContainerStats, criUtil cri.CRIClient) {
	for cid, stats := range containerStats {
		if stats == nil {
			klog.Warningf("Missing stats for container: %s", cid)
			continue
		}
		...
		if err != nil {
			klog.Errorf("Could not collect tags for container %s: %s", cid[:12], err)
		}
