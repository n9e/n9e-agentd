...
import (
	...
	cmetrics "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containers/metrics"
...
func updateContainerRunningCount(images map[string]*containerPerImage, c *containers.Container) {
	...
	if c.Excluded {
		...
		if err != nil {
			klog.Errorf("Cannot split the image name %s: %v", c.Image, err)
			return
		}
		...
		if err != nil {
			klog.Errorf("Could not collect tags for container %s: %s", c.ID[:12], err)
			return
		}
	...
	if d.instance.CollectImageSize {
		for _, i := range availableImages {
			if tags, ok := imageTagsByImageID[i.ID]; ok {
				...
				klog.V(6).Infof("Skipping image %s, no repo tags", i.ID)
			}
		}
	}
...
func (d *DockerCheck) Run() error {
	...
	for _, c := range cList {
		...
		if c.CPU != nil {
			...
			klog.V(5).Infof("Empty CPU metrics for container %s", c.ID[:12])
		}
		...
		if c.Memory != nil {
			...
			klog.V(5).Infof("Empty memory metrics for container %s", c.ID[:12])
		}

		if c.IO != nil {
			d.reportIOMetrics(c.IO, tags, sender)
		} else {
			klog.V(5).Infof("Empty IO metrics for container %s", c.ID[:12])
		}
		...
		if c.Network != nil {
			for _, netStat := range c.Network {
				if netStat.NetworkName == "" {
					klog.V(5).Infof("Ignore network stat with empty name for container %s", c.ID[:12])
					continue
				}
			...
			klog.V(5).Infof("Empty network metrics for container %s", c.ID[:12])
		}

		if collectingContainerSizeDuringThisRun {
			info, err := du.Inspect(c.ID, true)
			if err != nil {
				klog.Errorf("Failed to inspect container %s - %s", c.ID[:12], err)
			} else if info.SizeRw == nil || info.SizeRootFs == nil {
				klog.Warningf("Docker inspect did not return the container size: %s", c.ID[:12])
			} else {
		...
		if err != nil {
			...
			for _, stat := range stats {
				if stat.Name != docker.DataStorageName && stat.Name != docker.MetadataStorageName {
					klog.V(5).Infof("Ignoring unknown disk stats: %s", stat.Name)
					continue
				}
				if stat.Free != nil {
...
func (d *DockerCheck) Configure(config, initConfig integration.Data, source string) error {
	...
	if err != nil {
		klog.Warningf("Can't get hostname from docker, events will not have it: %s", err)
	}
	return nil
}
