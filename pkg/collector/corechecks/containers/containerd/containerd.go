...
import (
	...
	cutil "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containerd"
	ddContainers "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containers"
	cgroup "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util/containers/providers/cgroup"
	"k8s.io/klog/v2"
)
...
func (c *ContainerdCheck) Run() error {
	...
	if errHealth != nil {
		...
		klog.Infof("Error ensuring connectivity with Containerd daemon %v", errHealth)
		return errHealth
	}
...
func computeEvents(events []containerdEvent, sender aggregator.Sender, fil *ddContainers.Filter) {
	for _, e := range events {
		...
		if len(split) != 3 {
			...
			klog.V(5).Infof("Event topic %s does not have the expected format", e.Topic)
			continue
		}
		if split[1] == "images" {
		...
		if split[1] == "containers" || split[1] == "tasks" {
			...
			if err != nil {
				...
				klog.Errorf("Could not retrieve tags for the container %s: %v", e.ID, err)
			}
...
func computeMetrics(sender aggregator.Sender, cu cutil.ContainerdItf, fil *ddContainers.Filter) {
	...
	for _, ctn := range containers {
		...
		if err != nil {
			klog.Errorf("Could not retrieve the metadata of the container: %s", ctn.ID()[:12])
			continue
		}
		...
		if err != nil {
			klog.Errorf("Could not collect tags for container %s: %s", ctn.ID()[:12], err)
		}
		// Tagger tags
		...
		if errTask != nil {
			klog.V(6).Infof("Could not retrieve metrics from task %s: %s", ctn.ID()[:12], errTask.Error())
			continue
		}

		metrics, err := convertTasktoMetrics(metricTask)
		if err != nil {
			klog.Errorf("Could not process the metrics from %s: %v", ctn.ID(), err.Error())
			continue
		}
		...
		if err != nil {
			klog.Warningf("Could not retrieve OCI Spec from: %s: %v", ctn.ID(), err)
		}
		...
		if err != nil {
			klog.Errorf("Could not retrieve the size of the image of %s: %v", ctn.ID(), err.Error())
			continue
		}
		...
		if errTask != nil {
			klog.V(6).Infof("Could not retrieve pids from task %s: %s", ctn.ID()[:12], errTask.Error())
			continue
		}
		fileDescCount := 0
		for _, p := range processes {
			...
			if err != nil {
				klog.V(5).Infof("Failed to get file desc length for pid %d, container %s: %s", pid, ctn.ID()[:12], err)
				continue
			}
