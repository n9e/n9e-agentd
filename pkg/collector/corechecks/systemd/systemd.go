...
func (c *SystemdCheck) submitSystemdState(sender aggregator.Sender, conn *dbus.Conn) {
	...
	if err != nil {
		klog.V(5).Infof("err calling SystemState: %v", err)
	} else {
...
func (c *SystemdCheck) getPrivateSocketConnection(privateSocket string) (*dbus.Conn, error) {
	...
	if err != nil {
		klog.V(5).Infof("Error getting new connection using private socket %s: %v", privateSocket, err)
	}
	return conn, err
}
...
func (c *SystemdCheck) getSystemBusSocketConnection() (*dbus.Conn, error) {
	...
	if err != nil {
		klog.V(5).Infof("Error getting new connection using system bus socket: %v", err)
	}
	return conn, err
}
...
func (c *SystemdCheck) submitVersion(conn *dbus.Conn) {
	...
	if err != nil {
		klog.V(5).Infof("Error collecting version from the systemd: %v", err)
		return
	}
	checkID := string(c.ID())
	klog.V(5).Infof("Submit version %v for checkID %v", version, checkID)
	...
	for _, unit := range units {
		...
		if subStateMapping, found := c.config.instance.SubstateStatusMapping[unit.Name]; found {
			...
			if _, ok := subStateMapping[unit.SubState]; !ok {
				klog.V(5).Infof("The systemd unit %s has a substate value of %s that is not defined in the mapping set in the conf.yaml file. The service check will report 'UNKNOWN'", unit.Name, unit.SubState)
			}
...
func (c *SystemdCheck) submitBasicUnitMetrics(sender aggregator.Sender, conn *dbus.Conn, unit dbus.UnitStatus, tags []string) {
	...
	if err != nil {
		klog.Warningf("Error getting unit unitProperties: %s", unit.Name)
		return
	}
	activeEnterTimestamp, err := getPropertyUint64(unitProperties, "ActiveEnterTimestamp")
	if err != nil {
		klog.Warningf("Error getting property ActiveEnterTimestamp: %v", err)
		return
	}
...
func (c *SystemdCheck) submitPropertyMetricsAsGauge(sender aggregator.Sender, conn *dbus.Conn, unit dbus.UnitStatus, tags []string) {
	for unitType := range metricConfigs {
		...
		if err != nil {
			klog.Warningf("Error getting detailed properties for unit %s", unit.Name)
			return
		}
...
func sendServicePropertyAsGauge(sender aggregator.Sender, properties map[string]interface{}, service metricConfigItem, tags []string) error {
	if service.accountingProperty != "" {
		...
		if !accounting {
			klog.V(5).Infof("Skip sending metric due to disabled accounting. PropertyName=%s, AccountingProperty=%s, tags: %v", service.propertyName, service.accountingProperty, tags)
			return nil
		}
	}
