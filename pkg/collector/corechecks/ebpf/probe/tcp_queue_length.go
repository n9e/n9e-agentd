...
func (t *TCPQueueLengthTracer) Get() TCPQueueLengthStats {
	...
	if err != nil {
		klog.Errorf("Failed to get online CPUs: %v", err)
	...
	for it := t.statsMap.Iter(); it.Next(); {
		...
		if len(data) != C.sizeof_struct_stats_key {
			klog.Errorf("Unexpected tcp_queue_stats eBPF map key size: %d instead of %d.", len(data), C.sizeof_struct_stats_key)
			break
		}
		...
		if len(containerID) >= C.sizeof_struct_stats_key {
			klog.Warning("statsKey.cgroup_name wasn’t properly NULL-terminated")
			break
		}
		...
		if len(data) != C.sizeof_struct_stats_value*nbCpus {
			klog.Errorf("Unexpected tcp_queue_length eBPF map value size: %d instead of %d.", len(data), C.sizeof_struct_stats_value*nbCpus)
			break
		}
