...
func (k *KSMCheck) Run() error {
	...
	if !k.isCLCRunner || !k.instance.LeaderSkip {
		...
		if !config.Datadog.GetBool("leader_election") {
			return klog.Error("Leader Election not enabled. The cluster-agent will not run the kube-state-metrics core check.")
		}

		leader, errLeader := cluster.RunLeaderElection()
		if errLeader != nil {
			if errLeader == apiserver.ErrNotLeader {
				klog.V(5).Infof("Not leader (leader is %q). Skipping the kube-state-metrics core check", leader)
				return nil
			}
		...
		klog.V(6).Infof("Current leader: %q, running kube-state-metrics core check", leader)
	}
...
func (k *KSMCheck) Cancel() {
	klog.Infof("Shutting down informers used by the check '%s'", k.ID())
	k.cancel()
	k.CommonCancel()
}
...
func (k *KSMCheck) processMetrics(sender aggregator.Sender, metrics map[string][]ksmstore.DDMetricsFam, labelJoiner *labelJoiner) {
	for _, metricsList := range metrics {
		for _, metricFamily := range metricsList {
			...
			klog.V(6).Infof("KSM metric '%s' is unknown for the check, ignoring it", metricFamily.Name)
		}
	}
