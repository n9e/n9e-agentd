...
import (
	...
	corecfg "github.com/n9e/n9e-agentd/pkg/config"
	...
	coreutil "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/util"
...
func (o *OrchestratorCheck) Configure(config, initConfig integration.Data, source string) error {
	...
	if err != nil {
		_ = klog.Error("could not parse the config for the API server")
		return err
	}
...
func (o *OrchestratorCheck) Run() error {
	...
	if !o.isCLCRunner || !o.instance.LeaderSkip {
		...
		if !config.Datadog.GetBool("leader_election") {
			return klog.Error("Leader Election not enabled. The cluster-agent will not run the check.")
		}

		leader, errLeader := cluster.RunLeaderElection()
		if errLeader != nil {
			if errLeader == apiserver.ErrNotLeader {
				klog.V(5).Infof("Not leader (leader is %q). Skipping the Orchestrator check", leader)
				return nil
			}
		...
		klog.V(6).Infof("Current leader: %q, running the Orchestrator check", leader)
	}
...
func (o *OrchestratorCheck) processReplicaSets(sender aggregator.Sender) {
	...
	if err != nil {
		_ = klog.Errorf("Unable to process replica set list: %v", err)
		return
	}
...
func (o *OrchestratorCheck) Cancel() {
	klog.Infof("Shutting down informers used by the check '%s'", o.ID())
	close(o.stopCh)
	o.CommonCancel()
}
