...
func (ms *metricSender) getCheckInstanceMetricTags(metricTags []metricTagConfig, values *resultValueStore) []string {
	...
	for _, metricTag := range metricTags {
		...
		if err != nil {
			klog.V(5).Infof("metric tags: error getting scalar value: %v", err)
			continue
		}
		strValue, err := value.toString()
		if err != nil {
			klog.V(5).Infof("error converting value (%#v) to string : %v", value, err)
			continue
		}
...
func (ms *metricSender) reportScalarMetrics(metric metricsConfig, values *resultValueStore, tags []string) {
	...
	if err != nil {
		klog.V(5).Infof("report scalar: error getting scalar value: %v", err)
		return
	}
...
func (ms *metricSender) reportColumnMetrics(metricConfig metricsConfig, values *resultValueStore, tags []string) {
	...
	for _, symbol := range metricConfig.Symbols {
		...
		if err != nil {
			klog.V(5).Infof("report column: error getting column value: %v", err)
			continue
		}
		for fullIndex, value := range metricValues {
			...
			if _, ok := rowTagsCache[fullIndex]; !ok {
				...
				klog.V(5).Infof("report column: caching tags `%v` for fullIndex `%s`", rowTagsCache[fullIndex], fullIndex)
			}
...
func (ms *metricSender) sendMetric(metricName string, value snmpValueType, tags []string, forcedType string, options metricsConfigOption, extractValuePattern *regexp.Regexp) {
	if extractValuePattern != nil {
		...
		if err != nil {
			klog.V(5).Infof("error extracting value from `%v` with pattern `%v`: %v", value, extractValuePattern, err)
			return
		}
	...
	if forcedType == "" {
		...
		if err != nil {
			klog.V(5).Infof("metric `%s`: failed to get flag stream value: %s", metricFullName, err)
			return
		}
	...
	if err != nil {
		klog.V(5).Infof("metric `%s`: failed to convert to float64: %s", metricFullName, err)
		return
	}
	...
	default:
		klog.V(5).Infof("metric `%s`: unsupported forcedType: %s", metricFullName, forcedType)
		return
	}
}

func (ms *metricSender) gauge(metric string, value float64, hostname string, tags []string) {
	// we need copy tags before using sender due to https://github.com/n9e/n9e-agentd/staging/datadog-agent/issues/7159
