...
import (
	...
	"github.com/n9e/n9e-agentd/pkg/collector/runner"
...
func NewCollector(paths ...string) *Collector {
	...
	if pyVer != "" {
		klog.Infof("Embedding Python %s", pyVer)
		klog.V(5).Infof("Python Home: %s", pyHome)
		klog.V(5).Infof("Python path: %s", pyPath)
	}

	// Prepare python environment if necessary
	if err := pyPrepareEnv(); err != nil {
		klog.Errorf("Unable to perform additional configuration of the python environment: %v", err)
	}

	klog.V(5).Info("Collector up and running!")
	return c
}
...
func (c *Collector) RunCheck(ch check.Check) (check.ID, error) {
	...
	if ch.Interval() == 0 {
		...
		// checks.
		klog.Infof("Adding an extra runner for the '%s' long running check", ch)
		c.runner.AddWorker()
	} else {
