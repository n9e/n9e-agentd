...
func (p *integrationsCmd) newInstallCmd() *cobra.Command {
	cmd := &cobra.Command{
...
func (p *integrationsCmd) newRemoveCmd() *cobra.Command {
...
func (p *integrationsCmd) newListCmd() *cobra.Command {
	...
	cmd.Flags().BoolVarP(&p.versionOnly, "show-version-only", "q", false, "only display version information")
	return cmd
}
...
func (p *integrationsCmd) loadPythonInfo() error {
	...
	for {
		...
		if _, err := os.Lstat(agentReleaseFile); err == nil {
			p.reqAgentReleasePath = agentReleaseFile
			break
		}

		parentDir := filepath.Dir(p.rootDir)
		if parentDir == p.rootDir {
	...
	if p.pythonMajorVersion == "" {
		p.pythonMajorVersion = p.env.Agent.PythonVersion
	}

	p.constraintsPath = filepath.Join(p.rootDir, fmt.Sprintf("final_constraints-py%s.txt", p.pythonMajorVersion))
	if _, err := os.Lstat(p.constraintsPath); err != nil {
		return err
	}
...
func (p *integrationsCmd) detectPythonMinorVersion() error {
	if p.pythonMinorVersion == "" {
		pythonPath, err := p.getCommandPython()
		if err != nil {
			return err
		}
		...
		p.pythonMinorVersion = strings.TrimSpace(string(minorVersion))
	}
...
func (p *integrationsCmd) getCommandPython() (string, error) {
	if p.useSysPython {
		return pythonBin, nil
	}

	pyPath := filepath.Join(p.rootDir, p.getRelPyPath())
...
func (p *integrationsCmd) pip(args []string, stdout io.Writer, stderr io.Writer) error {
	pythonPath, err := p.getCommandPython()
	if err != nil {
		return err
	}
	...
	if p.verbose > 0 {
		args = append(args, fmt.Sprintf("-%s", strings.Repeat("v", p.verbose)))
	}
...
func (p *integrationsCmd) installCmd(pkg string) error {
	if err := p.loadPythonInfo(); err != nil {
		return err
	}

	err := validateUser(p.allowRoot)
	if err != nil {
		return err
	}
	...
	if p.localWheel {
		...
		// Install the wheel
		if err := p.pip(append(pipArgs, wheelPath), os.Stdout, os.Stderr); err != nil {
			return fmt.Errorf("error installing wheel %s: %v", wheelPath, err)
		}

		// Move configuration files
		if err := p.moveConfigurationFilesOf(integration); err != nil {
	...
	currentVersion, found, err := p.installedVersion(integration)
	if err != nil {
	...
	minVersion, found, err := p.minAllowedVersion(integration)
	if err != nil {
	...
	// Download the wheel
	wheelPath, err := p.downloadWheel(integration, semverToPEP440(versionToInstall), rootLayoutType)
	if err != nil {
	...
	shippedBaseVersion, found, err := p.installedVersion("datadog-checks-base")
	if err != nil {
...
func (p *integrationsCmd) downloadWheel(integration, version, rootLayoutType string) (string, error) {
	pyPath, err := p.getCommandPython()
	if err != nil {
		return "", err
	}
...
func (p *integrationsCmd) minAllowedVersion(integration string) (*semver.Version, bool, error) {
	lines, err := ioutil.ReadFile(p.reqAgentReleasePath)
	if err != nil {
...
func (p *integrationsCmd) installedVersion(integration string) (*semver.Version, bool, error) {
	pythonPath, err := p.getCommandPython()
	if err != nil {
		return nil, false, err
	}
...
func (p *integrationsCmd) moveConfigurationFilesOf(integration string) error {
	confFolder := p.env.Agent.ConfdPath
	...
	relChecksPath, err := p.getRelChecksPath()
	if err != nil {
		return err
	}
	confFileSrc := filepath.Join(p.rootDir, relChecksPath, check, "data")
...
func (p *integrationsCmd) remove(pkgs []string) error {
	if err := p.loadPythonInfo(); err != nil {
		return err
	}
	...
	pipArgs = append(pipArgs, pkgs...)
	pipArgs = append(pipArgs, "-y")

	return p.pip(pipArgs, os.Stdout, os.Stderr)
}

func (p *integrationsCmd) list() error {
	if err := p.loadPythonInfo(); err != nil {
		return err
	}
	...
	err := p.pip(pipArgs, io.Writer(pipStdo), os.Stderr)
	if err != nil {
		return err
	}
...
func (p *integrationsCmd) info(pkg string) error {
	...
	packageName := normalizePackageName(pkg)

	version, found, err := p.installedVersion(packageName)
	if err != nil {
