...
func (p *integrationsCmd) getRelPyPath() string {
	return filepath.Join(fmt.Sprintf("embedded%s", p.pythonMajorVersion), pythonBin)
}

func (p *integrationsCmd) getRelChecksPath() (string, error) {
	return filepath.Join(fmt.Sprintf("embedded%s", p.pythonMajorVersion), "Lib", "site-packages", "datadog_checks"), nil
}
