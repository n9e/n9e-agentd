...
func newStartServiceCmd(env *agent.EnvSettings) *cobra.Command {
...
func newStopServiceCmd(env *agent.EnvSettings) *cobra.Command {
	return &cobra.Command{
		Use:   "stop-service",
...
func newRestartServiceCmd(env *agent.EnvSettings) *cobra.Command {
	return &cobra.Command{
		Use:   "restart-service",
		Short: "restarts the agent within the service control manager",
		RunE: func(cmd *cobra.Command, args []string) error {
			err := StopService(config.ServiceName, true)
			if err != nil {
				return err
			}
...
func newInstallServiceCmd(env *agent.EnvSettings) *cobra.Command {
	return &cobra.Command{
		Use:   "install-service",
...
func newRemoveServiceCmd(env *agent.EnvSettings) *cobra.Command {
	return &cobra.Command{
		Use:   "remove-service",
...
func init() {
	agent.RegisterCmd([]agent.CmdOps{
		{CmdFactory: newStartServiceCmd, GroupNum: agent.CMD_G_GENERIC},
		{CmdFactory: newStopServiceCmd, GroupNum: agent.CMD_G_GENERIC},
		{CmdFactory: newRestartServiceCmd, GroupNum: agent.CMD_G_GENERIC},
		{CmdFactory: newInstallServiceCmd, GroupNum: agent.CMD_G_GENERIC},
		{CmdFactory: newRemoveServiceCmd, GroupNum: agent.CMD_G_GENERIC},
	}...)
}
