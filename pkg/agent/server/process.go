...
func (p *agentServer) startProcess() error {
	cf := p.config

	if !cf.ProcessConfig.Enabled {
		return nil
	}
	...
	if err := initInfo(cfg); err != nil {
		return fmt.Errorf("Error initializing info: %s", err)
	}

	if err := statsd.Configure(p.config.Statsd.Host, p.config.Statsd.Port); err != nil {
		return fmt.Errorf("Error configuring statsd: %s", err)
	}
	...
	sysprobeMonitor, err := heartbeat.NewModuleMonitor(heartbeat.Options{
		...
		TagVersion:         options.Version,
		TagRevision:        options.Revision,
	})
	go func() {
		<-p.ctx.Done()
		sysprobeMonitor.Stop()
	}()
	...
	cl, err := NewCollector(cfg, p.ctx)
	if err != nil {
		return fmt.Errorf("Error creating collector: %s", err)
	}
	if err := cl.run(); err != nil {
		return fmt.Errorf("Error starting collector: %s", err)
	}
...
func NewCollectorWithChecks(cfg *config.AgentConfig, checks []checks.Check, ctx context.Context) Collector {
	return Collector{
		ctx:           ctx,
...
func (l *Collector) runCheck(c checks.Check, results *api.WeightedQueue) {
	...
	for _, m := range messages {
		...
		extraHeaders.Set(headers.ProcessVersionHeader, options.Version)
...
func (l *Collector) run() error {
	...
	go func() {
		...
		tags := []string{
			fmt.Sprintf("version:%s", options.Version),
			fmt.Sprintf("revision:%s", options.Revision),
		}
		for {
			select {
			case <-heartbeat.C:
	...
	go func() {
		defer wg.Done()
		l.consumePayloads(processResults, processForwarder)
	}()

	wg.Add(1)
	go func() {
		defer wg.Done()
		l.consumePayloads(podResults, podForwarder)
	}()
