...
func (cf CloudFoundryConfigProvider) Collect() ([]integration.Config, error) {
	klog.V(5).Info("Collecting configs via the CloudFoundryProvider")
	...
	for _, desiredLRP := range desiredLRPs {
		...
		klog.V(5).Infof("Successfully got %d configs for app %s", len(newConfigs), desiredLRP.AppGUID)
...
func (cf CloudFoundryConfigProvider) getConfigsForApp(desiredLRP *cloudfoundry.DesiredLRP, actualLRPs []*cloudfoundry.ActualLRP) []integration.Config {
	...
	for adName, adVal := range desiredLRP.EnvAD {
		...
		for _, err := range errs {
			klog.Errorf("Cannot parse endpoint template for service %s of app %s: %s, skipping",
		...
		case vcOk:
			...
			if err != nil {
				klog.Errorf("Failed to render config for service %s of app %s: %s", adName, desiredLRP.AppGUID, err)
			} else {
				success = true
			}
...
func (cf CloudFoundryConfigProvider) assignNodeNameToNonContainerChecks(configs []integration.Config, desiredLRP *cloudfoundry.DesiredLRP, actualLRPs []*cloudfoundry.ActualLRP) {
	if len(actualLRPs) > 0 {
		aLRP := actualLRPs[0]
		klog.V(5).Infof("All non-container checks for app %s will run on Cell %s", desiredLRP.AppGUID, aLRP.CellID)
		for i := range configs {
			configs[i].NodeName = aLRP.CellID
		}
	} else {
		klog.Infof("No container running for app %s, checks for its non-container services will run on arbitrary node", desiredLRP.AppGUID)
	}
}
