...
func (c *FileConfigProvider) Collect() ([]integration.Config, error) {
	...
	for _, path := range c.paths {
		klog.Infof("%v: searching for configuration files at: %s", c, path)

		entries, err := readDirPtr(path)
		if err != nil {
			klog.Warningf("Skipping, %s", err)
			continue
		}
	...
	for _, conf := range defaultConfigs {
		if _, isThere := configNames[conf.Name]; !isThere {
			...
			klog.V(5).Infof("Ignoring default config file '%s' because non-default config was found", conf.Name)
		}
	}
...
func (c *FileConfigProvider) collectEntry(file os.FileInfo, path string, integrationName string) configEntry {
	...
	if fileName == "auto_conf.yaml" && containsString(config.C.IgnoreAutoconf, integrationName) {
		klog.Infof("Skipping 'auto_conf.yaml' for integration '%s'", integrationName)
	...
	if err != nil {
		klog.Warningf("%s is not a valid config file: %s", absPath, err)
	...
	klog.V(5).Info("Found valid configuration in file:", absPath)
	return entry
}
...
func (c *FileConfigProvider) collectDir(parentPath string, folder os.FileInfo) configPkg {
	...
	if filepath.Ext(folder.Name()) != dirExt {
		...
		klog.V(5).Infof("Not a config folder, skipping directory: %s", dirPath)
	...
	if err != nil {
		klog.Warningf("Skipping config directory %s: %s", dirPath, err)
		return configPkg{configs, defaultConfigs, otherConfigs}
	}
...
func GetIntegrationConfigFromFile(name, fpath string) (integration.Config, error) {
	...
	if strictErr := yaml.UnmarshalStrict(yamlFile, &cf); strictErr != nil {
		...
		klog.Warningf("reading config file %v: %v\n", fpath, strictErr)
	}
