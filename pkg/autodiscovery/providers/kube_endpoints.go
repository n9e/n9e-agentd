...
import (
	...
	"github.com/n9e/n9e-agentd/pkg/autodiscovery/common/utils"
...
func (k *kubeEndpointsConfigProvider) Collect() ([]integration.Config, error) {
	...
	for _, config := range parsedConfigsInfo {
		...
		if err != nil {
			klog.Errorf("Cannot get Kubernetes endpoints: %s", err)
			continue
		}
...
func (k *kubeEndpointsConfigProvider) invalidate(obj interface{}) {
	...
	if !ok {
		klog.Errorf("Expected a Service type, got: %T", obj)
		return
	}
	endpointsID := apiserver.EntityForEndpoints(castedObj.Namespace, castedObj.Name, "")
	klog.V(6).Infof("Invalidating configs on new/deleted service, endpoints entity: %s", endpointsID)
	k.Lock()
	defer k.Unlock()
...
func (k *kubeEndpointsConfigProvider) invalidateIfChangedService(old, obj interface{}) {
	...
	if !ok {
		klog.Errorf("Expected a Service type, got: %T", old)
		k.setUpToDate(false)
		return
	}
	...
	if valuesDiffer(castedObj.Annotations, castedOld.Annotations, kubeEndpointAnnotationPrefix) {
		klog.V(6).Info("Invalidating configs on service end annotations change")
		k.setUpToDate(false)
		return
	}
}
...
func (k *kubeEndpointsConfigProvider) invalidateIfChangedEndpoints(old, obj interface{}) {
	...
	if !ok {
		klog.Errorf("Expected an Endpoints type, got: %T", obj)
		return
	}
	// Cast the old object, invalidate on casting error
	castedOld, ok := old.(*v1.Endpoints)
	if !ok {
		klog.Errorf("Expected a Endpoints type, got: %T", old)
		k.setUpToDate(false)
		return
	}
	// Quick exit if resversion did not change
...
func parseServiceAnnotationsForEndpoints(services []*v1.Service) []configInfo {
	...
	for _, svc := range services {
		if svc == nil || svc.ObjectMeta.UID == "" {
			klog.V(5).Info("Ignoring a nil service")
			continue
		}
		...
		for _, err := range errors {
			klog.Errorf("Cannot parse endpoint template for service %s/%s: %s", svc.Namespace, svc.Name, err)
		}
...
func generateConfigs(tpl integration.Config, resolveMode endpointResolveMode, kep *v1.Endpoints) []integration.Config {
	if kep == nil {
		klog.Warning("Nil Kubernetes Endpoints object, cannot generate config templates")
	...
	default:
		klog.Warningf("Unknown resolve value: %s for endpoint: %s/%s - fallback to auto mode", resolveMode, namespace, name)
		fallthrough
