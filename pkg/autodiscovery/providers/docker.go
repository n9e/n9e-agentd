...
CONNECT:
	for {
		...
		if err != nil {
			klog.Warningf("error subscribing to docker events: %s", err)
		...
		for {
			...
			case ev := <-eventChan:
				...
				if ev.Action == "start" {
					...
					if err != nil {
						klog.Warningf("Error inspecting container: %s", err)
					} else {
						d.Lock()
			...
			case err := <-errChan:
				klog.Warningf("Error getting docker events: %s", err)
				d.Lock()
				d.upToDate = false
				d.Unlock()
...
func parseDockerLabels(containers map[string]map[string]string) ([]integration.Config, error) {
	...
	for cID, labels := range containers {
		...
		for _, err := range errors {
			klog.Errorf("Can't parse template for container %s: %s", cID, err)
		}
