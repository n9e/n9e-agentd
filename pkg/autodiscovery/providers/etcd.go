...
func NewEtcdConfigProvider(config config.ConfigurationProviders) (ConfigProvider, error) {
	...
	if len(config.Username) > 0 && len(config.Password) > 0 {
		klog.Info("Using provided etcd credentials: username ", config.Username)
...
func (p *EtcdConfigProvider) getIdentifiers(key string) []string {
	...
	if err != nil {
		klog.Error("Can't get templates keys from etcd: ", err)
		return identifiers
	}
...
func (p *EtcdConfigProvider) IsUpToDate() (bool, error) {
	...
	for _, identifier := range identifiers {
		if len(identifier.Nodes) != 3 {
			klog.Infof("%v does not have a correct format to be considered in the cache", identifier.Key)
			continue
		}
