...
func (z *ZookeeperConfigProvider) IsUpToDate() (bool, error) {
	...
	if z.cache.NumAdTemplates != len(identifiers) {
		if z.cache.NumAdTemplates == 0 {
			klog.Infof("Initializing cache for %v", z.String())
		}
	...
	if outdated > z.cache.LatestTemplateIdx || adListUpdated {
		klog.V(5).Infof("Idx was %v and is now %v", z.cache.LatestTemplateIdx, outdated)
		z.cache.LatestTemplateIdx = outdated
		klog.Infof("cache updated for %v", z.String())
		return false, nil
	}
	klog.Infof("cache up to date for %v", z.String())
	return true, nil
}
...
func (z *ZookeeperConfigProvider) getIdentifiers(key string) ([]string, error) {
	...
	for _, child := range children {
		...
		if err != nil {
			klog.Warningf("could not list keys in '%s': %s", nodePath, err)
			continue
		} else if len(nodes) < 3 {
			continue
		}
...
func (z *ZookeeperConfigProvider) getTemplates(key string) []integration.Config {
	...
	if err != nil {
		klog.Errorf("Couldn't get check names from key '%s' in zookeeper: %s", key, err)
		return nil
	}

	checkNames, err := parseCheckNames(string(rawNames))
	if err != nil {
		klog.Errorf("Failed to retrieve check names at %s. Error: %s", checkNameKey, err)
		return nil
	}

	initConfigs, err := z.getJSONValue(initKey)
	if err != nil {
		klog.Errorf("Failed to retrieve init configs at %s. Error: %s", initKey, err)
		return nil
	}

	instances, err := z.getJSONValue(instanceKey)
	if err != nil {
		klog.Errorf("Failed to retrieve instances at %s. Error: %s", instanceKey, err)
		return nil
	}
