...
func (l *KubeEndpointsListener) Listen(newSvc chan<- Service, delSvc chan<- Service) {
	...
	if err != nil {
		klog.Errorf("Cannot list Kubernetes endpoints: %s", err)
	}
...
func (l *KubeEndpointsListener) endpointsAdded(obj interface{}) {
	...
	if !ok {
		klog.Errorf("Expected an Endpoints type, got: %v", obj)
		return
	}
...
func (l *KubeEndpointsListener) endpointsUpdated(old, obj interface{}) {
	...
	if !ok {
		klog.Errorf("Expected an Endpoints type, got: %v", old)
		l.createService(castedObj, false, true)
		return
	}
...
func (l *KubeEndpointsListener) serviceUpdated(old, obj interface{}) {
	...
	if !ok {
		klog.Errorf("Expected a Service type, got: %v", obj)
		return
	}
	...
	if !ok {
		klog.Errorf("Expected a Service type, got: %v", old)
...
func (l *KubeEndpointsListener) endpointsForService(service *v1.Service) *v1.Endpoints {
	...
	if err != nil {
		klog.Warningf("Cannot get Kubernetes endpoints - Endpoints services won't be created - error: %s", err)
		return nil
	}
...
func (l *KubeEndpointsListener) endpointsDiffer(first, second *v1.Endpoints) bool {
	...
	if err != nil {
		klog.V(6).Infof("Cannot get Kubernetes service: %s", err)
		return true
	}
...
func (l *KubeEndpointsListener) createService(kep *v1.Endpoints, alreadyExistingService, checkServiceAnnotations bool) {
	...
	if err != nil {
		klog.V(5).Infof("Couldn't get standard tags for %s/%s: %v", kep.Namespace, kep.Name, err)
		tags = []string{}
	}
	...
	for _, ep := range eps {
		klog.V(5).Infof("Creating a new AD service: %s", ep.entity)
		l.newService <- ep
	}
}
...
func (l *KubeEndpointsListener) removeService(kep *v1.Endpoints) {
	...
	if ok {
		...
		l.m.Unlock()
		for _, ep := range eps {
			klog.V(5).Infof("Deleting AD service: %s", ep.entity)
			l.delService <- ep
		}
	} else {
		klog.V(5).Infof("Entity %s not found, not removing", kep.UID)
	}
}
