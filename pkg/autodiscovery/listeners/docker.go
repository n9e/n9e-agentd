...
func (l *DockerListener) Listen(newSvc chan<- Service, delSvc chan<- Service) {
	...
	if err != nil {
		klog.Errorf("can't listen to docker events: %v", err)
		...
		for {
			...
			case err := <-errs:
				if err != nil && err != io.EOF {
					klog.Errorf("docker listener error: %v", err)
					signals.ErrorStopper <- true
				}
				return
			}
...
func (l *DockerListener) init() {
	...
	if err != nil {
		klog.Errorf("Couldn't retrieve container list - %s", err)
	}
	...
	for _, co := range containersList {
		...
		if err != nil {
			klog.Errorf("Error getting check names from docker labels on container %s: %v", co.ID, err)
		}

		containerImage, err := l.dockerUtil.ResolveImageName(co.Image)
		if err != nil {
			klog.Warningf("Error while resolving image name: %s", err)
			containerImage = ""
		}
...
func (l *DockerListener) processEvent(e *docker.ContainerEvent) {
	...
	if found {
		...
		default:
			...
			klog.V(5).Infof("Expected die for container %s got %s: skipping event", cID[:12], e.Action)
			return
		}
	} else {
...
func (l *DockerListener) createService(cID string) {
	...
	if err != nil {
		klog.Errorf("Failed to inspect container '%s', not creating AD service, err: %s", cID[:12], err)
		return
	}
	...
	if l.filters.IsExcluded(containers.GlobalFilter, containerName, containerImage, "") {
		klog.V(5).Infof("container %s filtered out: name %q image %q", cID[:12], containerName, containerImage)
		return
	}
	...
	if err != nil {
		klog.Errorf("Error getting check names from docker labels on container %s: %v", cID, err)
	}
	...
	if err != nil {
		klog.Errorf("Failed to inspect container %s - %s", cID[:12], err)
	}
	_, err = svc.GetHosts()
	if err != nil {
		klog.Errorf("Failed to inspect container %s - %s", cID[:12], err)
...
func (l *DockerListener) removeService(cID string) {
	...
	if ok {
		...
		klog.V(5).Infof("Container %s not found, not removing", cID[:12])
	}
}
...
func (l *DockerListener) isExcluded(co types.Container) bool {
	...
	for _, name := range co.Names {
		if l.filters.IsExcluded(containers.GlobalFilter, name, image, "") {
			klog.V(5).Infof("container %s filtered out: name %q image %q", co.ID[:12], name, image)
			return true
		}
	}
	return false
...
func (s *DockerService) GetPorts() ([]ContainerPort, error) {
	...
	case cInspect.Config != nil && len(cInspect.Config.ExposedPorts) > 0:
		klog.Infof("using ExposedPorts for container %s as no port bindings are listed", s.cID[:12])
