...
func (l *ECSListener) refreshServices(firstRun bool) {
	...
	if err != nil {
		klog.Errorf("failed to get task metadata, not refreshing services - %s", err)
		return
	} else if meta.KnownStatus != "RUNNING" {
		klog.V(5).Infof("task %s is not in RUNNING state yet, not refreshing services", meta.Family)
		return
	}
	l.task = meta
	...
	for _, c := range meta.Containers {
		...
		if c.DockerID == "" {
			klog.V(5).Infof("Skipping a container for which ECS is reporting an empty ID: name %q, docker name: %q, image %q, image id: %q", c.Name, c.DockerName, c.Image, c.ImageID)
			continue
		}
		...
		if c.KnownStatus != "RUNNING" {
			klog.V(5).Infof("container %s is in status %s - skipping", c.DockerID, c.KnownStatus)
			continue
		}
		// Detect AD exclusion
		if l.filters.IsExcluded(containers.GlobalFilter, c.DockerName, c.Image, "") {
			...
			klog.V(5).Infof("container %s filtered out: name %q image %q", dockerID, c.DockerName, c.Image)
			continue
		}
		s, err := l.createService(c, firstRun)
		if err != nil {
			klog.Errorf("couldn't create a service out of container %s - Auto Discovery will ignore it", c.DockerID)
			continue
		}
		l.m.Lock()
...
func (l *ECSListener) createService(c v2.Container, firstRun bool) (ECSService, error) {
	...
	if err != nil {
		klog.Errorf("Error getting check names from docker labels on container %s: %v", c.DockerID, err)
	}
