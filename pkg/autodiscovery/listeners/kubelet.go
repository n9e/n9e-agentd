...
func (l *KubeletListener) createPodService(pod *kubelet.Pod, firstRun bool) {
	...
	if podIP == "" {
		klog.Errorf("Unable to get pod %s IP", pod.Metadata.Name)
	}
	...
	if len(ports) == 0 {
		...
		klog.V(5).Infof("No ports found for pod %s", pod.Metadata.Name)
	}
...
func (l *KubeletListener) createService(entity string, pod *kubelet.Pod, firstRun bool) {
	...
	for _, container := range pod.Status.GetAllContainers() {
		if container.ID == svc.entity {
			...
			if containerImage == "" {
				klog.V(5).Infof("couldn't find the container %s (%s) in the spec of pod %s", container.Name, container.ID, pod.Metadata.Name)
				containerImage = container.Image
			}

			// Detect AD exclusion
			if l.filters.IsExcluded(containers.GlobalFilter, container.Name, containerImage, pod.Metadata.Namespace) {
				klog.V(5).Infof("container %s filtered out: name %q image %q namespace %q", container.ID, container.Name, containerImage, pod.Metadata.Namespace)
				return
			}
			...
			if err != nil {
				klog.Warningf("Error while spliting image name: %s", err)
			}
	...
	podIP := pod.Status.PodIP
	if podIP == "" {
		klog.Errorf("Unable to get pod %s IP", podName)
	}
	...
	if len(svc.ports) == 0 {
		// Port might not be specified in pod spec
		klog.V(5).Infof("No ports found for pod %s", podName)
	}
	...
	if found && kubeletSvcEqual(old, &svc) {
		klog.V(6).Infof("Received a duplicated kubelet service '%s'", svc.entity)
		return
	}
	l.services[entity] = &svc
...
func (l *KubeletListener) removeService(entity string) {
	...
	if ok {
		...
		klog.V(5).Infof("Entity %s not found, not removing", entity)
	}
}
