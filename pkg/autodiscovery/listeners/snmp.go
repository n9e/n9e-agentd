...
import (
	...
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/persistentcache"
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/snmp"
...
type SNMPListener struct {
	...
	stop       chan bool
	config     snmptypes.ListenerConfig
...
func (l *SNMPListener) loadCache(subnet *snmpSubnet) {
	...
	if err != nil {
		klog.Errorf("Couldn't read cache for %s: %s", subnet.cacheKey, err)
		return
	}
	if cacheValue == "" {
		return
	...
	if err = json.Unmarshal([]byte(cacheValue), &devices); err != nil {
		klog.Errorf("Couldn't unmarshal cache for %s: %s", subnet.cacheKey, err)
		return
	}
...
func (l *SNMPListener) writeCache(subnet *snmpSubnet) {
	...
	if err != nil {
		klog.Errorf("Couldn't marshal cache: %s", err)
		return
	}

	if err = persistentcache.Write(subnet.cacheKey, string(cacheValue)); err != nil {
		klog.Errorf("Couldn't write cache: %s", err)
	}
}
...
var worker = func(l *SNMPListener, jobs <-chan snmpJob) {
	for {
		select {
		case <-l.stop:
			klog.V(5).Info("Stopping SNMP worker")
			return
		case job := <-jobs:
			klog.V(5).Infof("Handling IP %s", job.currentIP.String())
			l.checkDevice(job)
		}
	}
}
...
func (l *SNMPListener) checkDevice(job snmpJob) {
	...
	if err := params.Connect(); err != nil {
		klog.V(5).Infof("SNMP connect to %s error: %v", deviceIP, err)
		...
		if err != nil {
			klog.V(5).Infof("SNMP get to %s error: %v", deviceIP, err)
			l.deleteService(entityID, job.subnet)
		} else if len(value.Variables) < 1 || value.Variables[0].Value == nil {
			klog.V(5).Infof("SNMP get to %s no data", deviceIP)
			l.deleteService(entityID, job.subnet)
		} else {
			klog.V(5).Infof("SNMP get to %s success: %v", deviceIP, value.Variables[0].Value)
...
func (l *SNMPListener) checkDevices() {
	...
	for _, config := range l.config.Configs {
		...
		if err != nil {
			klog.Errorf("Couldn't parse SNMP network: %s", err)
			continue
		}
