...
import (
	...
	"github.com/n9e/n9e-agentd/pkg/autodiscovery/configresolver"
...
func (ac *AutoConfig) checkTagFreshness() {
	...
	for _, service := range servicesToRefresh {
		klog.V(5).Infof("Tags changed for service %s, rescheduling associated checks if any", service.GetTaggerEntity())
...
func (ac *AutoConfig) AddConfigProvider(provider providers.ConfigProvider, shouldPoll bool, pollInterval time.Duration) {
	...
	for _, pd := range ac.providers {
		if pd.provider == provider {
			...
			klog.Warningf("Provider %s was already added, skipping...", provider)
			return
		}
	}
...
func (ac *AutoConfig) GetAllConfigs() []integration.Config {
	...
	for _, pd := range ac.providers {
		...
		if err != nil {
			klog.V(5).Infof("Unexpected error returned when collecting configurations from provider %v: %v", pd.provider, err)
		}
...
func (ac *AutoConfig) processNewConfig(config integration.Config) []integration.Config {
	...
	if check.CollectDefaultMetrics(config) {
		...
		if len(metrics) == 0 {
			klog.Infof("%s doesn't have an additional metric configuration file: not collecting default metrics", config.Name)
		} else if err := config.AddMetrics(metrics); err != nil {
			klog.Infof("Unable to add default metrics to collect to %s check: %s", config.Name, err)
		}
	}
	...
	if config.IsTemplate() {
		...
		if err := ac.store.templateCache.Set(config); err != nil {
			klog.Errorf("Unable to store Check configuration in the cache: %s", err)
		}
	...
	if err != nil {
		klog.Errorf("Dropping conf for '%s': %s", config.Name, err.Error())
		return configs
	}
...
func (ac *AutoConfig) addListenerCandidates(listenerConfigs []config.Listeners) {
	...
	for _, c := range listenerConfigs {
		...
		if !ok {
			...
			klog.Warningf("Listener %s was not registered", c.Name)
			continue
		}
		klog.V(5).Infof("Listener %s was registered", c.Name)
...
func (ac *AutoConfig) initListenerCandidates() bool {
	...
	for name, factory := range ac.listenerCandidates {
		...
		case err == nil:
			...
			klog.Infof("%s listener successfully started", name)
		...
		case retry.IsErrWillRetry(err):
			...
			klog.Infof("%s listener cannot start, will retry: %s", name, err)
		default:
			// Log an error and remove from candidates
			klog.Errorf("%s listener cannot start: %s", name, err)
			delete(ac.listenerCandidates, name)
		}
	}
...
func decryptConfig(conf integration.Config) (integration.Config, error) {
	...
	if err != nil {
		return conf, fmt.Errorf("error while decrypting secrets in 'initConfig': %s", err)
	}
...
func (ac *AutoConfig) removeConfigTemplates(configs []integration.Config) {
	for _, c := range configs {
		if c.IsTemplate() {
			...
			if err != nil {
				klog.V(5).Infof("Could not delete template: %v", err)
			}
		}
	}
}
...
// for example when the `adIdentifiers` section of a config.yaml file contains
...
func (ac *AutoConfig) resolveTemplate(tpl integration.Config) []integration.Config {
	...
	for _, id := range tpl.ADIdentifiers {
		...
		for serviceID := range serviceIds {
			...
			if svc == nil {
				klog.Warningf("Service %s was removed before we could resolve its config", serviceID)
				continue
			}
...
func (ac *AutoConfig) GetLoadedConfigs() map[string]integration.Config {
	if ac == nil || ac.store == nil {
		klog.Error("Autoconfig store not initialized")
...
func (ac *AutoConfig) processNewService(svc listeners.Service) {
	...
	if err != nil {
		klog.Errorf("Failed to get AD identifiers for service %s, it will not be monitored - %s", svc.GetEntity(), err)
		return
	}
	for _, adID := range ADIdentifiers {
		...
		if err != nil {
			klog.V(5).Infof("Unable to fetch templates from the cache: %v", err)
		}
