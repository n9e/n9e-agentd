...
type CommonInstanceConfig struct {
	MinCollectionInterval int      `yaml:"minCollectionInterval" json:"minCollectionInterval"` // This changes the collection interval of the check - default: 15
	EmptyDefaultHostname  bool     `yaml:"emptyDefaultHostname" json:"emptyDefaultHostname"`   // This forces the check to send metrics with no hostname. This is useful for cluster-level checks.
	Tags                  []string `yaml:"tags" json:"tags"`                                   // A list of tags to attach to every metric and service check emitted by this instance, <key_1>:<value_1>
	Service               string   `yaml:"service" json:"service"`                             // Attach the tag `service:<SERVICE>` to every metric, event, and service check emitted by this integration.
