...
func (pd *configPoller) poll(ac *AutoConfig) {
	...
	for {
		...
		case <-pd.stopChan:
			...
			return
		case <-ticker.C:
			klog.V(6).Infof("Polling %s config provider", pd.provider.String())
			// Check if the CPupdate cache is up to date. Fill it and trigger a Collect() if outdated.
			upToDate, err := pd.provider.IsUpToDate()
			if err != nil {
				klog.Errorf("Cache processing of %v configuration provider failed: %v", pd.provider, err)
			}
			if upToDate == true {
				klog.V(5).Infof("No modifications in the templates stored in %v configuration provider", pd.provider)
				break
			}
			...
			if len(newConfigs) > 0 || len(removedConfigs) > 0 {
				klog.Infof("%v provider: collected %d new configurations, removed %d", pd.provider, len(newConfigs), len(removedConfigs))
			} else {
				klog.V(5).Infof("%v provider: no configuration change", pd.provider)
			}
...
func (pd *configPoller) collect() ([]integration.Config, []integration.Config) {
	...
	if err != nil {
		klog.Errorf("Unable to collect configurations from provider %s: %s", pd.provider, err)
		return nil, nil
	}
