...
func newNamedPipeListener(
	...
	klog.V(5).Infof("dogstatsd-named-pipes: %s successfully initialized", pipe.Addr())
	return listener, nil
}
		...
		case err.Error() == "use of closed network connection":
			{
				...
				klog.V(5).Info("dogstatsd-named-pipes: stop listening")
				return
			}
		default:
			klog.Error(err)
...
func (l *NamedPipeListener) listenConnection(conn net.Conn, buffer []byte) {
	klog.V(5).Infof("dogstatsd-named-pipes: start listening a new named pipe client on %s", conn.LocalAddr())
	startWriteIndex := 0
	for {
		...
		if err != nil {
			if err == io.EOF {
				klog.V(5).Infof("dogstatsd-named-pipes: client disconnected from %s", conn.LocalAddr())
				break
			}

			// NamedPipeListener.Stop uses a timeout to stop listening.
			if err == winio.ErrTimeout {
				klog.V(5).Infof("dogstatsd-named-pipes: stop listening a named pipe client on %s", conn.LocalAddr())
				break
			}
			klog.Errorf("dogstatsd-named-pipes: error reading packet: %v", err.Error())
