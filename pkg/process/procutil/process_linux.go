...
func (p *Probe) StatsForPIDs(pids []int32, now time.Time) (map[int32]*Stats, error) {
	...
	for _, pid := range pids {
		...
		if !util.PathExists(pathForPID) {
			klog.V(5).Infof("Unable to create new process %d, dir %s doesn't exist", pid, pathForPID)
			continue
		}
...
func (p *Probe) getCmdline(pidPath string) []string {
	...
	if err != nil {
		klog.V(5).Infof("Unable to read process command line from %s: %s", pidPath, err)
		return nil
	}
...
func bootTime(hostProc string) (uint64, error) {
	...
	if err != nil {
		klog.V(5).Infof("Unable to read stat file from %s: %s", filePath, err)
		return 0, nil
	}
