...
func (l *LocalResolver) Resolve(c *model.Connections) {
	...
	klog.V(6).Infof("ctrsByLaddr = %v", ctrsByLaddr)

	// go over connections again using hash computed earlier to resolver raddr
	for _, conn := range c.Conns {
		if conn.Raddr.ContainerId != "" {
			klog.V(6).Infof("skipping already resolved raddr %v", conn.Raddr)
			continue
		}
		...
		if cid, ok = ctrsByLaddr[addrWithNS{raddr, netns}]; ok {
			...
			klog.V(6).Infof("resolved loopback raddr %v to %s", raddr, cid)
		}

		if conn.Raddr.ContainerId == "" {
			klog.V(6).Infof("could not resolve raddr %v", conn.Raddr)
		}
