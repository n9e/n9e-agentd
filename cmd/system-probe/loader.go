...
func (l *Loader) Register(cfg *config.Config, httpMux *http.ServeMux, factories []api.Factory) error {
	for _, factory := range factories {
		if !cfg.ModuleIsEnabled(factory.Name) {
			klog.Infof("%s module disabled", factory.Name)
			continue
		}
		...
		if err != nil {
			klog.Errorf("new module `%s` error: %s", factory.Name, err)
			continue
		}

		if err = module.Register(httpMux); err != nil {
			klog.Errorf("error registering HTTP endpoints for module `%s` error: %s", factory.Name, err)
			continue
		}

		l.modules[factory.Name] = module

		klog.Infof("module: %s started", factory.Name)
	}
