...
import (
	...
	networkconfig "github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/network/config"
	"github.com/n9e/n9e-agentd/staging/datadog-agent/pkg/network/encoding"
...
func (nt *networkTracer) Register(httpMux *http.ServeMux) error {
	...
	httpMux.HandleFunc("/connections", func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			klog.Errorf("unable to retrieve connections: %s", err)
			w.WriteHeader(500)
			return
		}
	...
	httpMux.HandleFunc("/debug/net_state", func(w http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			klog.Errorf("unable to retrieve tracer stats: %s", err)
			w.WriteHeader(500)
			return
		}
	...
	time.AfterFunc(inactivityLogDuration, func() {
		if run := atomic.LoadUint64(&runCounter); run == 0 {
			klog.Warningf("%v since the agent started without activity, the process-agent may not be configured correctly and/or running", inactivityLogDuration)
		}
	})
...
func writeConnections(w http.ResponseWriter, marshaler encoding.Marshaler, cs *network.Connections) {
	...
	if err != nil {
		klog.Errorf("unable to marshall connections with type %s: %s", marshaler.ContentType(), err)
		w.WriteHeader(500)
		return
	}
	...
	klog.V(6).Infof("/connections: %d connections, %d bytes", len(cs.Conns), len(buf))
}
