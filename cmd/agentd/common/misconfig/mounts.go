	...
	if os.Geteuid() == 0 {
		...
		if caps, err := capability.NewPid2(0); err == nil && caps.Load() == nil {
			if caps.Get(capability.EFFECTIVE, capability.CAP_SYS_PTRACE) {
				klog.V(5).Infof("Running as root with cap_sys_ptrace - not checking for hidepid on proc fs")
				return nil
			}
		}
	}
...
func checkProcMountHidePid(path string, uid int, groups []int) error {
	...
	for scanner.Scan() {
		...
		if _, ok := mountOptsLookup["hidepid=2"]; !ok {
			...
			klog.V(6).Infof("Proc mounts hidepid=2 option is not set")
			return nil
		}
		...
		for _, gid := range groups {
			...
			if _, ok := mountOptsLookup[gidOpt]; ok {
				...
				klog.V(6).Infof("Proc mounts hidepid=2 with %s - proc fs inspection is enabled", gidOpt)
				return nil
			}
		}
