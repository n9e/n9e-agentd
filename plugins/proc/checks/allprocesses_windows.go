...
func getAllProcesses(probe *procutil.Probe) (map[int32]*procutil.Process, error) {
	...
	for success := w32.Process32First(allProcsSnap, &pe32); success; success = w32.Process32Next(allProcsSnap, &pe32) {
		...
		if !ok {
			...
			if err := cp.fillFromProcEntry(&pe32); err != nil {
				klog.V(5).Infof("could not fill Win32 process information for pid %v %v", pid, err)
				continue
			}
			cachedProcesses[pid] = cp
		} else {
			if err := cp.openProcHandle(pe32.Th32ProcessID); err != nil {
				klog.V(5).Infof("Could not reopen process handle for pid %v %v", pid, err)
				continue
			}
		}
		defer cp.close()
		...
		if err := windows.GetProcessTimes(procHandle, &CPU.CreationTime, &CPU.ExitTime, &CPU.KernelTime, &CPU.UserTime); err != nil {
			klog.V(5).Infof("Could not get process times for %v %v", pid, err)
			continue
		}

		var handleCount uint32
		if err := getProcessHandleCount(procHandle, &handleCount); err != nil {
			klog.V(5).Infof("could not get handle count for %v %v", pid, err)
			continue
		}

		var pmemcounter process.PROCESS_MEMORY_COUNTERS
		if err := getProcessMemoryInfo(procHandle, &pmemcounter); err != nil {
			klog.V(5).Infof("could not get memory info for %v %v", pid, err)
			continue
		}
		...
		if err := getProcessIoCounters(procHandle, &ioCounters); err != nil {
			klog.V(5).Infof("could not get IO Counters for %v %v", pid, err)
			continue
		}
	...
	for pid := range knownPids {
		...
		klog.V(5).Infof("removing process %v %v", pid, cp.executablePath)
		cp.close()
...
func getUsernameForProcess(h windows.Handle) (name string, err error) {
	...
	if err != nil {
		klog.V(5).Infof("Failed to open process token %v", err)
		return
	}
	defer t.Close()
...
func parseCmdLineArgs(cmdline string) (res []string) {
	...
	for _, b := range blocks {
		...
		if numquotes == 0 {
			...
			klog.Warningf("unexpected quotes in string, giving up (%v)", cmdline)
			return res
		}
...
func (cp *cachedProcess) fillFromProcEntry(pe32 *w32.PROCESSENTRY32) (err error) {
	...
	if usererr != nil {
		klog.V(5).Infof("Couldn't get process username %v %v", pe32.Th32ProcessID, err)
	}
	var cmderr error
	...
	if cmderr != nil {
		klog.V(5).Infof("Error retrieving full command line %v", cmderr)
...
func (cp *cachedProcess) openProcHandle(pid uint32) (err error) {
	...
	if err != nil {
		klog.V(5).Infof("Couldn't open process with PROCESS_VM_READ %v %v", pid, err)
		cp.procHandle, err = windows.OpenProcess(0x1000, false, uint32(pid))
		if err != nil {
			klog.V(5).Infof("Couldn't open process %v %v", pid, err)
			return err
		}
	}
	return
